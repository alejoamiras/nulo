...
9. **The live canaries** (all green = the redeploy gate): `bun run --cwd packages/bridge-core verify:l1 --config apps/tools/public/testnet-bridge.json --strict` · `BRIDGE_MANIFEST=apps/tools/public/testnet-bridge.json bun run --cwd apps/tools verify:deployments` · `fuel-testnet.ts` with `PRIVATE_RUNS=1` (the private self-paying claim MUST settle — re-confirm the step-6 gate first; this canary moves real Fee Juice) · **`fee-juice-canary-testnet.ts` (the DIRECT `feeJuice` lane — handler mint → `depositToAztecPublic(minFj)` → sponsored `FeeJuice.claim`; the lane the fueled smokes never exercise)** · a drip (`drip-canary-testnet.ts`) · the token list against its real origin (`TOKEN_LIST_LIVE=1` on `packages/bridge-core/src/token-list.test.ts` — origin, schema, chain filter, cache; never membership). **`PRIVATE_RUNS=1` is the settle-canary ONLY — its printed `minFuelFj` is a one-sample estimate; a one-sample number may only ever RAISE the floor, NEVER lower it.** Size `FUEL_SLICE_UNITS` so the live quote clears the manifest floor (the default quarter-unit was priced for an older pool; a slice under the floor reverts at the router's guard, `UniswapFuelSwap: insufficient output`, before anything moves). The private lane's ceiling is `Σ gasLimit·committedMaxFee` — every FPC self-pay claim MUST declare explicit `gasLimits` (`PRIVATE_HUB_CLAIM_GAS` for the hub claim, `PRIVATE_CLAIM_GAS` for the direct lane): a wallet given none declares the node's per-tx maximum (`txsLimits.gas`, 6.54M L2), which on the testnet fee schedule is ≈40 FJ and the FPC then refuses any floor-sized slice with `Amount too low to cover gas cost`. The sandbox cannot show this (its fees make the maximum cost nothing). To exercise the PRIVATE first claim (`register_token` paid by the bridged fuel through the FPC, then the claim paid from the credit the FPC kept) the validator must meet a token the hub does not know: `pre-create --no-register --seed-pool --token <fresh mintable>` then `fuel-testnet.ts --token <it> PUBLIC_RUNS=0 PRIVATE_RUNS=1` — with the public lane on, `register_and_claim_public` registers the token first and the private lane degrades to a plain claim. Never let two scripts sign from the deployer at once (a stuck nonce, a duplicated deploy). To change `l1.swap.minFuelFj`, run the default full calibration (`PRIVATE_RUNS` unset, ≥3 runs) — or leave the floor alone and note it as a follow-up.
