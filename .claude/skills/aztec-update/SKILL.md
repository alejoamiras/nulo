...
Confirm the classification (and, on a reset, the redeploy authorization) with the user before executing.
...
3. **L1 fuel**: `DeployFuelLive.s.sol` (guarded seeding — dry-run, then `--broadcast`; reuse flags `ROUTER_ADDRESS`/`FUEL_SWAP_ADDRESS` for partial recovery). The router binds the FeeJuicePortal at construction ⇒ always redeploys on a reset.
...
9. **The five live canaries** (all green = the redeploy gate): `verify-l1` · `verify:deployments` on the new pins · the candidate smoke (already done) · `fuel-testnet.ts` with `PRIVATE_RUNS=1` (the private self-paying claim MUST settle — it also prints a `minFuelFj` recalibration: apply it to the live manifest) · a drip.
