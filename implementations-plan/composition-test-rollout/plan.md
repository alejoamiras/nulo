...
### Phase 2 — FpcService composition test (fully-shallow target first — audit/VERIFICATION)
Fpc is fully shallow (no `simulate` anywhere) → lowest-risk first move. Harness: real `FpcService` + injected `FakeBrowserApi` + the shared fake via the factory + the **full required stub surface** (audit M1): `ProfileService` (`getActiveProfile`, `onProfileDeleted.add`), `NetworkService` (`getNetwork`/`getNode`, `registerChainPurgeSubscriber`), `TaskService` (`startNewTask`). Cover **`addFpc`** (shallow lookup + artifact validation + persistence) and **`getFpcs(chainId)`** (auto-discovery via the REAL pure `getContractInstanceFromInstantiationParams` + `registerContract`; the real lock + re-read prevents duplicate rows under repeated calls). Assert REAL state (storage rows, events), not the fake's returns.
...
### Phase 3 — TokenService composition test (shallow paths only)
Harness: real `TokenService` + real `OperationJournalService` (injected `FakeBrowserApi`) + the full stub surface (audit M1): `ProfileService` (`getActiveProfile`, `onProfileDeleted.add`), `NetworkService` (`getNetwork`/`getNode`, `registerChainPurgeSubscriber`), `AccountService`, `TaskService` (`startNewTask` — `parseTokenInterface` calls it). Target the **shallow** methods — `parseTokenInterface` (resolve instance/artifact, dedup via `getContracts`, real candidate extraction from the seeded REAL artifact, `registerContract` only when unregistered) and `getTokenInterface` (persisted selection preserved while candidates recompute). **Scope-out (hard, verified):** do NOT drive `addToken`/`fetchTokenMetadata`/`previewTokenMetadata` — those reach `simulate(...)` (deep; e2e). Assert REAL state.
...
### Phase 4 — DappSession no-PXE harness (LAST, droppable)
...
### Phase 5 — Limits doc + CLAUDE.md pointer + drift-guard finalize
