...
- [x] E — Backup-restore config allowlist (F-06) — **PR #270 merged**; ConfigService.restore typed allowlist (skips strictSecurityMode/sessionTtl/dev/unknown). Gate: full test 2669 · config test 2 · lint 0 · smoke 69-pass (1 CI-skipped flake). Belt deferred to L.  *(prerequisite of L)*
...
- [x] L — Bearer redesign + memory hygiene (F-11) — **PR #271 merged**; `SessionSecretBox` random-token wrapped-secret bearer (HKDF→AES-GCM, AAD=profile.id) replaces the password-equivalent passhash; `restore()` drops the unseal callback + fail-closed shape gate (legacy passhash/strict+bearer/bad-unwrap → silentClose); `clearBearer`; `fromPassword` scratch-wipe. **No-re-registration invariant VERIFIED**: key-vectors byte-identical 9/9 + integration test proves a pre-F-11 legacy-passhash session silentCloses then the SAME profile re-unlocks with the SAME password. Gate: wallet-crypto 30 · full test 2684 · typecheck:all/lint 0-err · proverless e2e 70-pass/1-skip (baseline parity)
- [x] PROMOTE — **PR #272 merged into `dev`** (2026-07-13, merge commit `cc0e7b2`, GitHub `verified:true`). Two post-impl `origin/dev` merges (`#276` deletion subsystem, `#277` residual cleanups) + **four `gpt-5.6-sol` xhigh audits** → 8 residual findings fixed (1 CRITICAL self-inflicted empty-name authwit bypass, 3 HIGH, 2 MEDIUM, 2 LOW), 7 surfaced as dev-subsystem follow-ups (`audit-gpt56sol.md`). All three required gates green: `quality-status` · `network-e2e-status` (5 shards + canary + 2 heavy) · `smoke-e2e-status`. Merged via the clean sign + squash-auto-merge path (signed HEAD `db481ff` → web-flow-signed squash), **no `--admin`**.
