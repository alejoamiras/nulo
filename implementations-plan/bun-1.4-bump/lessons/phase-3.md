...
| `string_decoder` | 1.3.0 → 1.1.1 | `readable-stream@2`'s `~1.1.1` | **The bundle-reachable one** (`node-stdlib-browser` → `^1.0.0`, via `vite-plugin-node-polyfills`). `bun pm diff string_decoder@1.1.1 1.3.0`: `lib/string_decoder.js` is UNCHANGED — the delta is a deleted `.travis.yml` + package.json metadata. **Closure delta (codex post-impl catch)**: the dep range change (`safe-buffer ~5.2`→`~5.1`) makes the chain resolve a nested `safe-buffer@5.1.2` (bun.lock:2491) where 5.2.1 sat before. `bun pm diff safe-buffer@5.1.2 5.2.1`: the ONLY behavioral line is `SafeBuffer.prototype = Object.create(Buffer.prototype)` (absent in 5.1.2) — `string_decoder` never touches SafeBuffer's prototype chain (it only calls the copied static `Buffer.from`/`allocUnsafe`), so the delta is characterized-and-inert for this consumer. Full battery + build green over it. |
