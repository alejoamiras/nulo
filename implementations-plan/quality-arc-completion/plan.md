...
| 4 | **Q6** activity-feed extraction | mid → **codex-narrowed** | **Codex (sess 019eeb77) narrowed to behaviour-safe subset:** DO **#1 `useIncomingTransfers`** (inject clients — parent owns lifecycle; composable wires listeners + `refresh()` + `dispose()` removing handlers; hiding client creation changes the page's auto-connect fetch timing) + **#2 `buildJournalAwaitingCardProps`** (WIDGET-LOCAL helper, preserve widget amount-gating `amountRaw!==undefined` + independent symbol — NOT terminal's). **DEFER #3** buildActivityRows-reuse (deterministic tie-break: widget seeds journal→tx→incoming vs helper tx→journal→incoming → reorders at slice boundary; + `filterPendingDoubleRender` input diff) **and #4** template-collapse (hot-path fallback branches). Don't touch good helpers. |
