...
| 6 (in progress) | **Q8** popup form abstractions | mid → **codex-narrowed** (sess 019eeba7) | **SHIP:** (1a) `useFormState.rebase()` + per-field `field.isDirty` (baseline init=initial → backward-compat) + adopt in SIMPLE edit dialogs (EditEndpoint, EditNetwork, EditAccount); (b) fix the 2 real list-sync DRIFTS directly — `SelectTokenPopup` missing update handler + `BalanceView` divergent delete. **DEFER:** 1b FormPopup-Enter (NewEndpoint submits on any-Enter; guard must be subtree+input-target, behaviour change); broad `useEntityCrud` popup migration (eager fetch/subscribe on setup vs always-mounted PopupManager → lifecycle change — needs `enabled`/start-stop first); EditContact rebase (per-field dirty drives warning display + sender-state baseline — risky, separate). Tests: dirty/reset/cross-field-validation after rebase. |
