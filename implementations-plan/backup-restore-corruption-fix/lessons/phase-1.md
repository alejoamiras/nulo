...
- `useFullBackupImport.ts`: tx-restore provenance filter right after account restore — `importedAddresses = new Set(newAccounts.filter(a => !a.restoreError).map(a => a.address))`; `data.transaction` filtered to keep only txs whose `account` ∈ that set; foreign/failed-account txs are dropped-and-recorded into `restoreErrorLog.value.transaction` ("Transaction references an account not imported from this backup").
