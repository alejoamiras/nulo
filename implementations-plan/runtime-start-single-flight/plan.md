...
**Overlap-on-retry, settled across both audit legs:** codex initially demanded `allSettled` quiescence (a fast BB rejection must not reset the memo while config still ran). Fable then showed that with the BB veto in place `allSettled` protects an EMPTY set — a BB rejection keeps the memo (no retry, no overlap) and `config.load` cannot reject in production (it swallows its errors) — while ADDING a silent forever-pending boot when a leg hangs after the other failed, which is the defect class this arc removes. Resolution: `Promise.all` with the veto in the BB leg's `.catch`; pinned (fast rejection + memo kept + no re-run while config pends).

**What remains genuinely retryable** (fable's exhaustive enumeration, adopted): transient schema-status storage writes — the retry pin drives exactly that representative. The reset also covers any future throwing pre-registration step at zero cost.

**Fable's plain-memo alternative (F3), considered and NOT taken:** dropping the retry zone entirely (pure `memo ??= doStart()`) would fix both proven defects by deletion. Not adopted because the goal explicitly prescribes the failure-resetting form, the residual retryable zone is real if small, and the veto classification is now fully pinned; recorded here so a future simplification has the argument ready.
