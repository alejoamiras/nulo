...
- `scripts/ci-cd/complexity-baseline.test.ts` (runs in `test:ci-gating`, already wired into `_unit-tests.yml`) enforces: exact per-rule × per-file count match (grow = forbidden, shrink = regenerate the manifest in the same PR), Biome-version pin match (a Biome bump forces deliberate regeneration — scores drift between releases; the same function scored 89 under eslint-plugin-sonarjs 4.2 and 135 under Biome 2.5.1), and zero file-wide `biome-ignore-all` suppressions.
