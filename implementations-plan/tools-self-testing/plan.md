...
| 32 | Outbox round trip: not consumed at proposal → after finalization → lands (positive only: the automine local network proves at proposal, so the unproven refusal is not observable — arc-2 codex round 1) | – | – | ✓ | – |
...
**Adopted from the arc-2 codex round 1 (not ready for PR, 13 findings)**: every cell asserts its advertised payer — cells 1/2 pay from private credit with the exact ceiling deducted, cell 27 pays authwit + exit from the actor's public Fee Juice with the exact fee sum, the tampered registration rides each fee mode; deposits/exits tests take an actor per test; cells 20/21 gain their private half through one shared private-gas leg; cell 31 gains the L1 factory switch (owner = the deployer) with `finally` restore; cell 32 becomes positive-only (the automine network proves at proposal — an unobservable negative is not coverage); the routeless send is refused for its empty route under a real floor and cell 17 asserts the router's `insufficient fuel`; the race test accepts either winner; forge always builds incrementally; child logs persist under `~/.cache/nulo-bridge-sandbox/logs` and CI uploads them; the legacy `deploy:sandbox` alias calls the CLI's `main()`; the exit reaper skips groups that already exited. Not adopted: a signer mutex — every flow awaits each L1 write and the one concurrent shape uses two keys, so the handle documents one writer per key per process (viem's `nonceManager` was tried and reverted: it counts past a send that reverts at gas estimation); the "printed admin key" — nothing prints one.
