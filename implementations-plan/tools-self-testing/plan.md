...
4. **Delivery**: the FIRST time any PR is opened. `gh stack sync` if `dev` moved, `gh stack submit --auto`, `gh pr edit` each body (what/why, gates run, the runbook step for arc 1), `gh pr checks --watch`; a red new-workflow check is fixed on its arc branch and re-pushed. Update `implementations-plan/index.md`. Never merge; never run `required-checks.sh --apply` (owner-only).
