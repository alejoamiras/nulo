...
- **Spec correction discovered on the first post-fix run**: strict security mode (default ON) drops the session on an SW kill — the sw-resilience pins prove cold-restore never re-derives the bearer — so the replayed discovery lands on a LOCKED wallet and is correctly QUEUED, not popped. The spec gained the unlock-drain leg (unlock via the liveness probe popup after boot; the drain pops discover). This also means the fix and F-B16's clean-loss semantics compose exactly as designed: the relay saves the message across the wake, the queue holds it across the lock, the drain delivers it.
- Post-fix GREEN (corrected spec): kill → click-as-wake → relay buffer → boot proven → unlock → drain → discover → approve → verify → **connected**. Exit 0, single pass.
- **Source-swap RED (corrected spec, wiring reverted to `origin/dev`): CONFIRMED** — alarms cleared, SW asserted dead at click, boot proven complete, wallet unlocked — and still no discover popup in 60s (retried ×2): pre-fix the waking message dies before any listener exists, so there is nothing for the unlock-drain to deliver. The RED/GREEN pair now differs ONLY in the two wired files.
