...
**Audit cycle:** [`audit-codex.md`](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/journal-stage-restructure/audit-codex.md) + [`audit-opus.md`](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/journal-stage-restructure/audit-opus.md) — both `approve-with-fixes`. Convergent findings on (a) coverage gap was framed wrong (BLOCKER), (b) Phase A tests must extend `TransactionCardLayout.test.ts` not just `TransactionAwaitingCard.test.ts`, (c) FSM has 8 stages including `queued`, (d) Phase D must exclude `tx-sendTx-reject`. Codex additionally caught the JobStage type reuse + retry/gate conflict + suggested a focused helper. All applied. Details in [§12](#12-audit-response-log).
...
| Helper signature | **Focused, single-purpose**: `waitForSendTxActiveStage(walletPopup)`. Not a generic stringly helper — codex's defensive suggestion. Future tests that want a different stage must add a new helper, visible in diff. |
| Stage choice | **Active stage** (explicit allowlist of `simulating \| proving \| submitting`). Originally planned as `proving`-only; broadened during impl (see [`lessons/phase-impl.md`](lessons/phase-impl.md)) because empirically on local WASM the wallet spends most wall-time in `simulating`. Codex post-impl audit then re-tightened the selector from "anything past pending/queued" to the explicit 3-stage allowlist so terminal states never silently match. |
