...
- **Teardown deletion (primary)**: the marker's stored `tabId` (§N-04(1)) is the deletion key — `tabs.onRemoved` supplies ONLY a tabId, and pre-establishment there is no `ActiveSession` to map through (final-pass B2: origin-based deletion was unimplementable as specced). Tab close deletes every marker whose `tabId` matches. (Implementation deviation, deliberate: the cross-origin-nav branch does NOT delete markers — markers are origin-less so the branch cannot discriminate them, same-origin SPA `loading` events would misfire, and request-keying already removes any consequence; a navigated tab's marker dies via the TTL.) An immediate reconnect reads a clean set regardless — request-keyed markers were never readable by a new handshake. The wired lifecycle (approve → tab close → reconnect) is unit-tested against the real tab-lifecycle deps shape.
