...
## Round 3 — FINAL fresh-context pass (NEW session 019eae2a, zero prior involvement)

Verdict: **conditional approve** — all four conditions folded same-round (exit-witness-bound withdraw completion; envelope-authenticated sealerL1 + cache risk re-scoped, ledger L14; stale/stale-deployment unified; A2/A3 as release-gate approvals). No ledger reversal — "The phased design is executable... I would tighten D3's trust-cache posture, not the unified-form/journal architecture."

- **HIGH** — Withdraw completion can still be spoofed. [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:70) only checks `to === L1_PORTAL` plus `recipient/amount`. That does not bind the consume tx to the specific `exitTxHash`; two withdraws with the same recipient and amount let a forged `consumeTxHash` on record B pass using record A’s real consume. Fix: before `completedAt`, recompute the expected L2→L1 message/witness from record B’s `exitTxHash` and compare the tx calldata’s epoch/leafIndex/path or an equivalent derived message identity. Current live consume flow has no stronger identity anchor yet in [useWithdraw.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/faucet/src/composables/useWithdraw.ts:170).

- **HIGH** — The seal-trust cache is still trusted more than the threat model admits. [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:61) and [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:148) call cache poisoning “deletion-equivalent”; it is not. A forged positive entry can force future one-sign private deposits and strand fresh funds after an irreversible L1 deposit. This is worse because `sealerL1` remains outside the authenticated envelope in [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:33) and is then used for mismatch/revocation gating in [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:62). Fix: put `sealerL1` inside the v2 envelope, verify it post-unseal, and treat the cache as an explicit testnet-risk acceptance or demote it to opt-in/session-scoped trust.

- **MEDIUM** — One contradiction survived the audit folds. D1/D5 introduce distinct `stale-deployment` attention in [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:47) and [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:80), but P2 proof ⑪ still expects `attention: stale` in [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:103). That is an implementation trap. Fix: choose one enum and align prose/tests.

- **MEDIUM** — A2/A3 are slightly misframed. A2 is not a normal UX choice; it is a one-time acceptance of legacy private recipient tamper risk in [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:57). A3 is only acceptable for explicitly internal testnet use because the plan intentionally retains sealed blobs until Clear/7-day prune in [plan.md](/Users/alejoamiras/Projects/nulo/nulo-4/implementations-plan/bridge-ux-trust/plan.md:82). Those need release-gate wording, not “recommended default” framing.

- Ledger re-litigation: no full L1-L13 reversal. The phased design is executable. The part I would tighten is D3’s trust-cache posture, not the unified-form/journal architecture.

conditional approve (with conditions: bind withdraw completion to the exact exit witness/message, authenticate `sealerL1` inside the private envelope and re-scope the seal-trust cache as explicit testnet risk, unify `stale` vs `stale-deployment`, and make A2/A3 explicit release-gate approvals)

## Round 4 — post-impl audit (NEW session, dir codex-MLB3ytTp)

Initial verdict: **reject** (3 blocking findings — false-done on rediscovered private claims; unfinished-junk cap eviction; generic-fingerprint trust reuse). All three fixed in `5ed9831` with test pins; verdict-flip resume appended below when received.

1. High: false completion is still possible for rediscovered private claims. In [useBridgeJournal.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/faucet/src/composables/useBridgeJournal.ts:385), `finishDepositByReceipt()` marks a record done unless `recordMessageConsumed()` returns `false`; but `recordMessageConsumed()` returns `null` whenever the secret is unavailable or the re-simulate fails for any non-`message gone` reason ([useBridgeJournal.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/faucet/src/composables/useBridgeJournal.ts:427)). After a refresh, private records with only `sealedEnvelope` have no cached secret, so a forged `claimTxHash` can be auto-finished on any successful receipt check. The test suite currently pins this unsafe behavior instead of rejecting it ([useBridgeJournal.test.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/faucet/src/composables/useBridgeJournal.test.ts:193)). This breaks the D4 “no fake done” claim and can later let prune delete a still-live bearer record.

2. High: the cap can still evict a live unfinished record. `capRecords()` keeps “unfinished first”, but it does not distinguish genuine unfinished records from attacker-injected unfinished junk; it just sorts all unfinished by `updatedAt` and slices ([journal.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/bridge-core/src/journal.ts:112)). A localStorage attacker can therefore inject >100 syntactically valid unfinished records and push out a real in-flight private deposit, destroying the only sealed blob through the cap path the plan claimed was closed. The only retention test covers completed junk, not unfinished junk ([journal.test.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/bridge-core/src/journal.test.ts:145)).

3. High: the trust-cache mitigation is weaker than the plan. `providerFingerprint()` collapses every unrecognized injected wallet into `"injected"` ([useDeposit.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/faucet/src/composables/useDeposit.ts:47)), and `isSealTrusted()` accepts any exact string match ([seal-trust.ts](/Users/alejoamiras/Projects/nulo/nulo-4/packages/bridge-core/src/seal-trust.ts:51)). That means switching from one unrecognized injected wallet to another still reuses trust, even though the plan required “unknown ⇒ re-run self-test”. This can strand future private deposits without any storage attacker. There is no test pin for the `"unknown"/"injected"` case.
