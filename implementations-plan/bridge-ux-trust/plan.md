...
- leafIndex resolution order: envelope → L1 receipt of `depositTxHash` (`parseEventLogs(InboxAbi, "MessageSent")` — chain-authoritative, covers the crash-between-receipt-and-reseal window) → never bare localStorage. **This rule is scoped to v2 records.** Legacy exception (codex BLOCKER + fable #3): pre-arc records have `leafIndex` but never persisted `depositTxHash`, so `legacy-unauthenticated` records may use their bare stored `leafIndex` AND unauthenticated `recipient` once, behind a loud warning that names both — safe because leafIndex tamper is liveness-only, and the legacy recipient risk is exactly today's (pre-arc) exposure, accepted once to avoid stranding in-flight funds.
...
- Migration (runs once at journal init, parse-isolated per record, write-before-delete): legacy public deposit ⇒ keyed by computed `secretHashHex`; legacy private (`sealedSecret`) ⇒ `legacy-unauthenticated` attention, never auto-continues, manual accept-and-finish with a loud warning; secret-less/garbage ⇒ `stale` (discard-only); legacy public deposit with NO `leafIndex` (killed pre-receipt) ⇒ unrecoverable under the new model: `stale`, discard-only copy, never a forever-polling Claim; legacy withdraw ⇒ keyed by `exitTxHash`, `consumeTxHash` carried (its prompt-free wait auto-finishes); if BOTH the journal and legacy keys exist ⇒ journal wins, legacy ignored after one logged warning. Migrated records are never `sessionLive`. Toast the count. (fable #7)
...
Smallest proof (jsdom localStorage + mocked Contract/node/wallet, fake timers): ① migration of all legacy shapes + corrupt JSON; ② **no-auto-claim regression** — rediscovered claimable deposit + connected wallets ⇒ zero send/sign calls; ③ sessionLive record auto-continues through gate→send; ④ per-record dedup + two-id concurrency; ⑤ claimTxHash persisted → receipt success ⇒ completedAt; dropped ⇒ re-claimable; **unrelated balance change clears nothing** (HIGH-b pin); ⑥ envelope-recipient swap ⇒ no send, `tampered`, display resynced (HIGH-a pin); ⑦ unseal failure ⇒ trust revoked + record intact; ⑧ per-record mismatch refusal; ⑨ **signature counting**: first private deposit exactly 2 signs then 0 at claim; second deposit exactly 1; post-refresh resume exactly 1; ⑩ rediscovered consumeTxHash ⇒ receipt awaited, `writeContract` never called; ⑪ stale-deployment record (mismatched portal/bridge) ⇒ `attention: stale`, watchers skip, actions disabled; ⑫ receipt lookup that throws/stays unknown past the budget ⇒ `attention: unknown-outcome`, record intact, manual Retry re-enters the wait; ⑬ legacy-unauthenticated record claims with its bare stored leafIndex behind the warning (BLOCKER fix pin); ⑭ promptLanes: two sessionLive records reaching their promptful send ⇒ the second awaits the first per wallet lane, while prompt-free polls interleave (fable #2); ⑮ a provisional `wd-pending-*` record rediscovered after reload ⇒ `unknown-outcome`, discard-only (fable #6); ⑧ additionally pins: wrong connected L1 account ⇒ pre-unseal `mismatch`, NO trust revocation; revocation fires only when connected === `sealerL1` (fable #4); ⑯ same address + DIFFERENT provider fingerprint ⇒ self-test re-runs (no trust hit); ⑰ tx-identity: a `claimTxHash` whose receipt succeeds but whose record-message still simulates claimable ⇒ `unknown-outcome`, not done; a consume tx with wrong target/args ⇒ `unknown-outcome`; ⑱ cap-priority: junk flood never evicts an unfinished record.
...
**Asks (user):** resolved in Phase 0 — multi-entry ✓, separate L1-mint CTA with explicit L1-vs-L2 copy ✓, fold HIGHs ✓, same-session-only auto ✓, branch off dev post-#78 ✓. Remaining at the approval gate — **A2 and A3 are explicit sign-off decisions, not defaults** (codex double-audit HIGH): **A1** done-card retention (L2 — retain-until-Clear, both auditors now agree) · **A2** legacy private records: manual accept-and-finish behind a loud warning that names the unauthenticated recipient+leafIndex (recommended) vs discard-only — this knowingly permits ONE last use of pre-arc unauthenticated metadata · **A3** CSP/security headers deferred to the hardening pass (recommended) — note that active XSS remains the only theft-capable path, so deferral means "trusted internal testnet use" until that pass lands.
