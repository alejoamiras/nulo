...
- leafIndex resolution order: envelope → L1 receipt of `depositTxHash` (`parseEventLogs(InboxAbi, "MessageSent")` — chain-authoritative, covers the crash-between-receipt-and-reseal window) → never bare localStorage. **Legacy exception** (codex contradiction-check BLOCKER): pre-arc records have `leafIndex` but never persisted `depositTxHash`, so `legacy-unauthenticated` records may use their bare stored `leafIndex` once, behind the loud legacy warning — safe because leafIndex tamper is liveness-only (a wrong leaf fails the message-hash check; it cannot redirect).
...
- **Stale-deployment refusal** (L4, made concrete — codex contradiction-check HIGH): on resume/action, compare the record's `(chainId, portal, bridge)` against the live `bridge-deployments` config; mismatch ⇒ `attention: stale`, watchers/pollers skip it, Claim/Finish disabled (Discard-only). Applies to PUBLIC records too — the private path additionally gets it for free via the recovery-key message binding.
...
Unresolved disputes carried to the audits: none — L2 resolved at contradiction-check. Contradiction-check round 1 (codex): 1 BLOCKER + 2 HIGH + 1 NIT, all folded (legacy-leafIndex exception in D2; receipt-fallback state machine in D4; stale-deployment refusal concretized in D5/P2; L2 retention pin in P3).
