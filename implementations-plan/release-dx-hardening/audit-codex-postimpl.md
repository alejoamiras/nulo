...
5. **Permissions broader than needed** (workflow-wide `contents/pull-requests/issues: write` inherited by build/test jobs). **FIXED in the `/harden security` pass:** top-level dropped to `contents: read` (the least-privilege default — covers checkout; artifact/cache ops need no scope), with write escalated only on the four jobs that write (release-please, auto-unstick, attach-assets, sync). This also REVERSED the `issues: write` I'd added in the /code-review round: GitHub's `pull-requests: write` scope explicitly covers PR labels + comments, and these jobs operate on PRs, so `issues: write` was unnecessary — removed from auto-unstick + sync. (release-please keeps its documented recommended set + uses the App token regardless.)
