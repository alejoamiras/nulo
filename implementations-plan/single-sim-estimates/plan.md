...
- **Status**: **APPROVED by owner 2026-08-10** (rev 3.2; codex `approve`; Asks: 1 ACCEPT — after the trade was re-explained with the failure-case-only framing; 2 ACCEPT; 3 ACCEPT; 4(a) ACCEPT — owner provided an AZTEC account secret (extension export) + Aztec testnet RPC rather than a Sepolia L1 key: B1 first imports it via deriveNuloAccountKeys and checks its existing private-FJ balance — funded ⇒ funded shapes proceed without the L1 route; unfunded ⇒ surface immediately for a real Sepolia key or fall to 4(b) A2-deferral. Secret stored in gitignored packages/bridge-core/.env, marked BURNED-AFTER-USE — it was disclosed in chat) — implementation may begin at C1
...
- **eli5_mode**: file
- **Recon**: [recon.md](recon.md) (incl. audit erratum) · **Audits**: [audit-codex.md](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/single-sim-estimates/audit-codex.md) (r1 `reject` — both blockers verified, adopted) · [audit-fable.md](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/single-sim-estimates/audit-fable.md) (r1 `conditional approve` — all 4 conditions adopted; advisories F-5–F-10 adopted) · **Binding precedent**: `implementations-plan/fee-estimation-speedup/` charter + ledger
...
### Asks — ALL RESOLVED at gate (owner approval 2026-08-10; details in the Status line)
...
- **Codex (r1, fresh, xhigh)**: `reject` — 2 blockers (sequencing, measurement gaps incl. PrivateFPC funding) + H1 fj fold + H2 Ask-1 backstop + H3 probe — ALL adopted. [audit-codex.md](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/single-sim-estimates/audit-codex.md)
- **Fable (r1, fresh)**: `conditional approve` — 4 conditions (probe respec, Inference-1 fact + resequencing, injection story, F-4 rule) + advisories F-5–F-10 — ALL adopted. Sponsored pin verified airtight. [audit-fable.md](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/single-sim-estimates/audit-fable.md)
- **Codex (final fresh-context pass on rev 2)**: `reject` — Critical (Ask-4(b) fallback non-executable: P2 sims need funded private FJ, no unfunded path; fragmented-note canary requirement silently dropped), H1 (standalone inner-hash authwit class undetectable at estimate — Ask 1 must name it + fixture), H2 (probe route unpinned through the shared `buildAndEstimate` dep — split typed deps + options-pins), M1 (clamp blast radius incl. embedded/NO_FROM + RPC posture via retained `txsLimits`). ALL adopted into rev 3 (ledger #11–14). [audit-codex.md](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/single-sim-estimates/audit-codex.md)
