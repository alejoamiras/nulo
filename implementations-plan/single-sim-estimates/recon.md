...
## ERRATUM (audit round 1)

- **Fact 10 correction (fable F-6)**: upstream `EmbeddedWallet.sendTx` sizes AND sends from a **stubbed**, payload-inclusive, `skipTxValidation: true` sim (`embedded_wallet.js:85-160` — `buildAccountOverrides` swaps the stub class), then clamps via `getGasLimits(gasUsed, maxTxGasLimits, 0.1)`. The original claim ("unstubbed forEstimation sim") was wrong; the correction is conservative-in-our-favor (upstream's shipped posture is exactly stub-sized + clamped + padded). Also: upstream's 0.1 pad is multiplicative (`mul(1+pad)`), not additive.
- **Inference-1 class question RESOLVED as fact (both auditors, from source)**: an unstubbed sim hard-fails on missing authwits BEFORE any result exists — `UtilityExecutionOracle.getAuthWitness` throws (`@aztec/pxe/dest/.../utility_execution_oracle.js:266-271`), reached from the real account's `verify_private_authwit` via the `aztec_utl_getAuthWitness` oracle; `skipTxValidation` only gates the post-execution `node.isValidTx` (`pxe.js:796`). Discovery REQUIRES the stub. Corollary (fable, from aztec-nr v5.0.1 Noir source): `assert_current_call_valid_authwit` emits its `CallAuthorizationRequest` offchain effect UNCONDITIONALLY (even with a valid witness attached) — call-shaped authwit ops always surface effects under a stub; `assert_inner_hash_valid_authwit` standalone (intent-hash kind) emits NOTHING.
- **Validation-drop side fact (fable)**: `skipTxValidation: false` costs one `node.isValidTx` RPC per estimate sim (`pxe.js:796-806`) — dropping it is a small unclaimed win.
