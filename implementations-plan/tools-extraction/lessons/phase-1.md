...
- **Reclassification.** Recon sorted plans by topic; the review tested what each plan's own commits changed outside the tools roots. Eight had changed code that stays (any-erc20-bridge #544, bridge-permit2-recipient-commitment #260, bridge-private #78, dedup-bridge-conductors #377, operator-gates #525, private-fuel, swap-fuel, private-fuel-fee-fix) → restored with their index lines; 28 stay deleted. `tools-two-network` stays deleted: its only wallet diff is a 4-line lock-test tweak.
