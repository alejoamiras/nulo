...
**Status:** ✅ **COMPLETE (2026-07-03)** — v0.24.0 shipped end-to-end, **zero `--admin`**. All 4 goal elements verified: (1) Release `isPrerelease:false` + chrome/firefox zips + SHASUMS; (2) `faucet.nulo.sh` on the new testnet (`chainId 2793892258`); (3) `nulo.sh` serves `v0.24.0`; (4) `main→dev` synced (prerelease manifest `{".":"0.24.0"}`). **`AUTO_UNSTICK_ENABLED` stayed OFF** (manual unstick + manual self-signed sync — both proven live). The landing needed a **manual CF-API prod deploy** (`refresh-landing` skips on `workflow_dispatch` by design — see `lessons/phase-2.md`). codex audit done (reject → all 5 findings verified + folded, incl. Phase 0 pre-flight blockers).
...
- **Satisfy main's `strict: true` FIRST (dev is behind main by 2).** main has `#173` ("rename required-check aggregators (main)", 3 lines / 3 workflow files) that dev lacks; `git merge-base --is-ancestor origin/main origin/dev` → NO. main's protection is `strict: true` (branch-up-to-date) + the 3 required checks. So the promote PR would be "out of date". **Bring main→dev first:** try `gh pr update-branch` on the promote PR (merges main→dev in one op), or a manual `sync/main-to-dev-*` PR merged with `--merge` (commitlint-skip #223 applies) — either way the 3-way merge keeps main's `#173` (dev didn't touch those files; no revert; the promote preview was conflict-free).
...
- **Validation gate** — `git merge-base --is-ancestor <v0.24.0 sha> origin/dev` → ancestor (dev has main's 0.24.0 release commit); `git show origin/dev:.release-please-prerelease-manifest.json` = `{".":"0.24.0"}`; a `release-please … --dry-run` on dev now previews the next rc cleanly (**actual: `0.24.1-rc.0`**, not the seed's guessed `0.25.0-rc.0` — the only commit on dev since v0.24.0 is a `fix:` (#249 chain-guard) → patch, not minor; the *mechanism* is correct, dev anchors at 0.24.0). The sync merged with plain `--merge`, no `--admin`. Layers: lint (commitlint-skip fired) + live dry-run.
