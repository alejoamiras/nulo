...
- **profileId source — ownerless shapes get it threaded** (codex round 1: authwits and balances carry NO profileId, transaction's is optional/backup-controlled): the composable passes the authoritative `createdProfileId` (`useFullBackupImport.ts:685`) as a new REQUIRED parameter to the `restore` RPCs of `auth-registry`, `token-balance`, and `transaction` (spec + client + call-site updates; the fence keys on the passed id, never on row fields). Codex round 2 adds: (a) the messaging base's `restore(...args: unknown[])` (`packages/extension-messaging/src/background/client.ts:147-149`) enforces no arity — each concrete CLIENT gets an exact typed `restore(rows, profileId)` declaration so omission fails compilation at the composable; (b) RPC dispatch has no parameter-schema validation (`core/base-service.ts:105-125`) — each service VALIDATES `profileId` (non-empty string) before capture and fails closed, otherwise a missing value fences the `undefined` map key and silently defeats the fence. This is a new RPC input (not a new untrusted source or storage field): the value is popup-supplied like the rows themselves and is used only as a fence key. Services whose rows carry a validated `profileId` (`contact`, `token`, `account`, `network`, `fpc` — verified per site at implementation; any found ownerless joins the threaded group) fence on the row-scope id they already use.
...
6. Round 2 (APPROVE-WITH-CHANGES, adopted): `restore(rows, profileId)` REQUIRED with no legacy form; exact typed override on each concrete client (base `restore(...args: unknown[])` enforces no arity); fail-closed `profileId` validation at each service boundary before capture (dispatch has no schema validation); recorded as a new RPC input, not a new trust surface.
