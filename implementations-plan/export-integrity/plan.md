...
Fixes **N-01 (Critical)** — the full-backup export page has no re-entry latch, no error boundary, and a stale-checksum window — and **N-13** — backup/account file readers lack size caps (decompression-bomb surface). Spec: `implementations-plan/audit-448-remediation/runbook.md`; verdicts: `audit/bugs/2026-08-22-production-ready/adjudication-2026-08-24.md`; recon: [recon.md](./recon.md); audits: [audit-codex.md](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/export-integrity/audit-codex.md), [audit-fable.md](https://github.com/alejoamiras/nulo/blob/9f11de70b13933be2d54c3eb79622b1ff2719aba/implementations-plan/export-integrity/audit-fable.md). Base: dev `ea9be876`. Tier: **mid** (rubric: security-sensitivity HIGH; 1 high → mid). Revision 2 (post round-1 dual audit).
...
2. **`full-backup-helpers.ts`**: `export const MAX_BACKUP_FILE_BYTES` — 16 MiB at plan time (codex R1's memory-amplification attack), **recalibrated to 64 MiB in Phase 3** after measurement showed a FRESH wallet's encrypted artifact is already ~22.4 MiB (ledger + lessons/phase-3.md; codex resume: approve); `readBackupFile` pre-checks `file.size` → parseError `"Backup File Too Large"` / `"The backup file is too large to import. Please select a correct backup file."` through the existing parseError channel.
...
_Gate passed: lint exit 0, vue-tsc clean, 47/47 tests green (files.test.ts + node-env files.caps.test.ts real-stream bomb pins + helpers size gate). Streams present in BOTH envs — the jsdom skipIf never skips._
...
| **Cap recalibrated 16 → 64 MiB** after e2e measurement: a FRESH wallet's encrypted artifact is 23,443,420 B (~22.4 MiB) — the 16 MiB estimate was below reality and the export gate fired on legitimate backups (loudly, as designed). 64 MiB = ~3× fresh headroom, bomb still finitely bounded; parse-amplification documented as estimate | measurement + codex resume (final-pass session) | **adopted — codex: approve** |
