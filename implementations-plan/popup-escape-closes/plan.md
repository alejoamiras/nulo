...
tier: light
driver: claude-code
status: completed 2026-09-23 — merged to dev (#675, squash `5b447f45`) on the owner's authorisation ("Let's please babysit this merge, you are explicitly authorized to merge."). History: all phases ✓, codex post-impl loop converged (conditional approve → approve, 2 rounds); PR into dev opened, NOT merged (the owner decides). First CI run red on one Chrome network shard (`pointerClick` pressed a popup mid-slide): fixed in the helper, codex rounds 3–5 on the fix converged (conditional → conditional → approve). The owner's manual smoke then found Escape with a menu open inside a popup closed the whole toolbar popup (dev does the same): fixed in `DropdownRoot`, codex consult converged (conditional → approve; its popup guard dropped, D9). Approved 2026-09-23 as rev 3, no conditions
eli5_mode: artifact
code_review: off
...
| A fee-method menu open inside the two authwits popups | Escape closes the menu (its own handling) but leaves the key unhandled, so Chrome's toolbar popup closes the whole wallet; in a tab the popup's trap is then the top one and a second Escape releases it silently | first Escape closes the menu and is marked handled, so the wallet stays open; second closes the popup |
| Any open menu on the shared dropdown, on pages too | Escape closes the menu, and Chrome's toolbar popup then closes the whole wallet | Escape closes only the menu |
...
**Owner sign-off for the menu rows (recorded):** on 2026-09-23, after the manual smoke, asked whether to apply the menu fix ("With it, Escape on an open menu on a page closes only the menu, not the whole extension"), the owner wrote: "Yes, please ask Codex to see what it thinks about the fix, and then apply it."
...
| `apps/extension/src/components/ui/Dropdown/DropdownRoot.vue` | its Escape handler calls `preventDefault()` before closing the menu (added after the owner's manual smoke; see *A menu inside a popup* and D9) |
| `apps/extension/src/components/ui/Dropdown/Dropdown.test.ts` | Escape closes the menu and comes back `defaultPrevented` |
...
| `apps/extension/tests/e2e/network/popup-escape-layered.test.ts` (new) | on the local network: Settings → Advanced → Account state → Authwits → actions menu → "toggle registry" opens `change_authwits_registry`; open its fee-method menu; Escape closes the menu while the popup stays *open* (the keyboard is still contained: Tab never reaches the page's `authwits-actions-btn`); Escape again closes the popup (`settleClosedPopup` before asserting it is gone); each press is read for `defaultPrevented`: handled, handled, then unhandled once nothing is open |
...
**A menu inside a popup.** `FeeSettingsCard` (inside `ChangeAuthwitsRegistryPopup.vue:119` and `RevokeAuthwitsPopup.vue:215`) renders a `Dropdown`. On open, `DropdownRoot` registers its own document keydown handler (`:121`) and then installs its own trap (`:145`), which becomes the top of the stack and pauses the popup's. On Escape, the menu's keydown handler runs first and calls `close()`. For a real key event the browser runs a microtask checkpoint after each listener, so Vue's flush tears the menu trap down right there: its `checkEscapeKey` is removed before its turn, and the popup trap's listeners are re-installed on `document` during that same invocation, which the DOM does not extend to listeners added mid-dispatch. So the popup trap does not see this Escape, and neither trap marks it handled. Chrome's toolbar popup closes the whole wallet on an unhandled Escape (the owner's manual smoke found it; a tab shows nothing), so the menu's handler calls `preventDefault()` itself (D9). The second Escape reaches the popup trap. The network e2e proves the layering and reads each press for `defaultPrevented`.
...
| D9 | The menu marks its own Escape handled (`preventDefault()` in `DropdownRoot`); the popup does not skip an already-handled Escape | a popup guard on `event.defaultPrevented`; the menu's trap owns Escape and closes from `onDeactivate`; register the trap before the keydown listener; `stopImmediatePropagation` | the owner's smoke: an unhandled Escape closes Chrome's toolbar popup. The guard protects no current path (the popup's trap is paused while a menu is open) and would keep a popup open over a focused disabled `Input`, which prevents every key; a trap-owned Escape loses dismissal when the trap fails to activate; reordering keeps the timing dependence; `stopImmediatePropagation` does not prevent the browser's default (codex consult, `lessons/phase-4.md`) |
...
**Owner's manual smoke (2026-09-23).** In the real Chrome toolbar popup, Escape with the fee-method menu open inside the registry popup closed the whole wallet: the menu left the key unhandled, and dev does the same. Fixed in `DropdownRoot` (D9) after a codex consult (conditional → approve in two rounds; its popup guard dropped). Firefox's toolbar panel closes on every Escape, handled or not (Mozilla bug 1443758, WONTFIX), so there the feature shows only in a tab or window. Follow-ups, not taken here: a Firefox preview build for every extension PR (`pr-quick.yml` builds Firefox only when a diff touches its build files), and `PasskeyCeremonyDialog`'s Escape, which cancels without marking the key handled. `lessons/phase-4.md` § Owner's manual smoke. Both are taken, with the `webdriver-ownership.test.ts` flake, in [popup-escape-followups](../popup-escape-followups/plan.md).
