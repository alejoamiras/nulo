...
**Status:** review complete 2026-09-06 · implementation not started · owner-approved to run 2026-09-07.
...
Five sequential PRs into `dev`, each from a fresh `dev` pull after the previous one merged. Slug ==
worktree dir == plan dir == branch suffix (`worktree-<slug>`). Phase 1 needs no blueprint; the rest run
`/blueprint` at the tier shown, inside the phase worktree.
...
Mark a row ✓ by replacing ☐ with `✓ #<PR>` once the PR is merged.
...
- **PR** only after the loop converged. Squash into `dev`. Conventional title from the table. Body: ids
  addressed, ids skipped with reasons, net LOC, the local-gate output.
...
- **Merge:** authorized below. `gh pr merge <n> --squash --delete-branch`, then `git checkout dev &&
  git pull`, `agent-worktree done <slug>`, mark the row ✓ with the PR number, print
  `LESSONS_FILE=implementations-plan/<slug>/lessons/phase-N.md`.
- **Lessons:** every codex consult with its verdict, every skipped id, every flake and re-run.

## Owner authorizations (2026-09-07)

- **Merge** the P1–P5 PRs into `dev` once `quality-status`, `network-e2e-status` and `smoke-e2e-status`
  are green and the codex loop converged. No `--admin`. No promote to `main`, no release, no rc cut.
...
- **Pre-answers to blueprint's Phase 0 questions** (do not block on `AskUserQuestion`; use these):
  ...
  recon; skip the reuse sweep) · foreign-reviewer effort `high` · single-arc per phase, no stack ·
  worktree homing yes, slug from the table (this instruction is the standing authorization for
  `EnterWorktree` on the `dedup-*` slugs) · ELI5 companion: append one paragraph to this README's phase
  row instead of publishing a new artifact.
...
```
/goal Every row P1–P5 in implementations-plan/dedup-ledger/README.md's phase table reads `✓ #<PR>`, each backed in the transcript by: the phase's local gate (`bun run lint`, `bun run typecheck:all`, `bun run test`) reporting exit 0 in the phase worktree; a codex fix loop over the phase's net diff (`/codex high`, adversarial + no-over-engineering + comment-quality ask, fixes committed, the same session RESUMED for re-review) that converged — the converging pass's "no new material findings" quoted; for P2–P5 a `/blueprint <tier from the table>` run inside the phase worktree under the README's pre-answers whose final codex audit reached approve (or conditional-approve with every condition adopted) and whose plan.md phases are all ✓; `LESSONS_FILE=implementations-plan/<slug>/lessons/phase-N.md` printed per phase; the PR (squash into dev, the table's conventional title, opened only after the loop converged) showing every required check green without `--admin` and `gh pr view <n> --json state,mergedAt` reporting MERGED; phases done strictly in order P1→P5, each from a fresh `git checkout dev && git pull`; no Tier-4 (deferred) id touched; every skipped id logged in lessons with its reason.
```

**Fallback — `/loop 15m`** (interval-driven):

```
/loop 15m Drive implementations-plan/dedup-ledger forward, phase by phase (P1→P5), never idle. Each firing: 1) Reality check — read README.md's phase table and the current phase's lessons (authoritative, not the chat); `git status`, `git log --oneline -5`; a PR open? `gh pr view --json number,state,statusCheckRollup` (no --watch). 2) No phase in flight → start the next: `git checkout dev && git pull`, home into the slug's worktree, P1 direct / P2–P5 `/blueprint <tier>` with the README's pre-answers; implement only the phase's ledger ids. 3) After each meaningful edit run `bun run lint` plus the touched package's tests; commit small, conventional, signed. 4) Diff complete → local gate (`bun run lint && bun run typecheck:all && bun run test`) → `/codex high` fix loop, resuming the same session each round, until a round reports no new material findings → open the PR with the table's title → `gh pr checks --watch`. 5) Red check: flake → re-run once; red twice → fix or hold. Green → `gh pr merge <n> --squash --delete-branch`, mark the row `✓ #<n>`, `agent-worktree done <slug>`, print `LESSONS_FILE=…`. 6) Stuck, or a decision you would normally bring to me → `/codex high` with full context, decide, log it in lessons; five failures on one step → reassess with codex. Hard limits: never `--admin`, never main or release branches, never a Tier-4 id, never a user-visible change, never weaken a gate. All five rows ✓ → write the wrap-up (what shipped per phase, skipped ids, every contentious codex call with the options and why) and stop.
```
