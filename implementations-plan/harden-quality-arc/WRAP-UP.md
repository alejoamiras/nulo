...
The arc's **code** landed via GitHub-signed squash-merges (finding PRs #188–#219, all `verified`). The local `docs(quality-arc)` tracker commits on `dev-quality` are **unsigned** (AFK / 1Password-outage window). `dev`'s ruleset is **squash-only**, so merging this promote collapses the arc into one GitHub-re-signed commit — the unsigned tracker commits do **not** block the merge. (Optional: `git rebase --exec 'git commit --amend --no-edit -S' <base>` on `dev-quality` first if you want its own history signed; requires force-push, so only before merge.)
