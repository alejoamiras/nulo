...
- **Phase 2** — migrate the ~33 method-call read sites `managers.X.foo()` → `requireX().foo()` (mechanical; grep-driven, file-by-file). LEAVE the 5 tolerant sites + 6 assignment sites. After each cluster: `bun run lint` + `typecheck:all` (vue-tsc) + touched units. Behavior-preservation check per site: confirm it's unlock-guarded (auth route / post-bootstrap); a pre-unlock method-call path would be a pre-existing latent bug → BUG-PIN, don't "fix".
