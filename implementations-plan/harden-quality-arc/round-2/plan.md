...
- **R1.1 ⏳ IN FLIGHT (#230)** — helpers landed: `restoreRows` + `nextNumericId`/`nextRandomId` + `requireOwnedRow` + 17 unit tests; no consumer yet (behavior-preserving). (auth-registry `max==nextNumericId` characterization lands with the R1.5 adoption.)
