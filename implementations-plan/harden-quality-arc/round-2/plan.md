...
- **R1.2 ⏳ PR #231 (@ `e2c01ea`)** — contact migrated onto the helpers (least divergent, proves the pattern): 3 ownership guards → `requireOwnedRow`, addContact → `nextRandomId`, restore → `restoreRows`. Behavior-preserving (25 pass + 3 pins incl. the isolation suite; typecheck 0). Full gate running.
