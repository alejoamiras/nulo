...
- **R1.3b ⏳ PR #233 (network)** — 11 guards → `requireOwnedRow` (getActiveNetwork return-null KEPT); `_freshStored8` → `nextRandomId`; **restore LEFT hand-rolled** (untrusted `unknown[]` + cross-row collision + schema — special case like account). 52 network unit tests + network isolation assertion + typecheck 0. Full gate running.
