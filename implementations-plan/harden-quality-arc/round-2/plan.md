...
- **R1.3a ✓ (fpc) — PR #232 @ gate** — fpc migrated (5 guards → `requireOwnedRow`, 2 create-loops → `nextRandomId`, restore → `restoreRows` w/ legacy-reject-via-throw). fpc isolation assertion added (getFpc foreign-reject; fpc has no unit tests). Full gate running.
