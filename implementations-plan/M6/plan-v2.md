# M6 — Vue design system + component library (post-dual-audit)

> **Status**: draft v2, post audit by Codex xhigh + Claude general-purpose. Awaiting user approval.
>
...
- 14 New<X>/Edit<X> popup pairs (~250 lines each) — no `EntityForm` abstraction
...
- 8 of 19 `popup/components/modules/` SFCs are pure presentational (no service/store imports) and should be L3, not L4
...
- Visual sandbox (**Histoire** with hard decision gate; fallback = **Storybook 10 Vue3-Vite**, NOT a custom page)
- Automated visual regression (**Lost Pixel** against built Histoire stories) — mandatory from Phase 2
...
- **14 New/Edit popup pairs** (7 New + 7 Edit) + 1 unpaired `EditProfilePopup` + 1 unpaired `NewSenderPopup`
...
Pure modules to re-classify L4 → L3 in Phase 5 (Claude verified, all have ZERO service/store imports):
1. `popup/components/modules/activity/TransactionAwaitingCard.vue`
2. `popup/components/modules/activity/TransactionsList.vue`
3. `popup/components/modules/capabilities/CapabilityDetailPanel.vue`
4. `popup/components/modules/general/EmojiGrid.vue`
5. `popup/components/modules/general/WarningView.vue`
6. `popup/components/modules/send/AmountCard.vue`
7. `popup/components/modules/send/FeeJuiceCard.vue`
8. `popup/components/modules/send/SendTypesCard.vue`
...
7. **Granular stop rule** (replaces v1's blanket two-failure rule):
   - typecheck/lint failure: fix and retry once
   - unit/component test failure: halt the sub-PR (not the arc)
   - e2e or manual smoke 2× consecutive failures: halt the arc, post audit-and-iterate
   - visual regression diff > tolerance: halt the sub-PR; manual decision before continuing
...
[L3] composites        src/components/composite/  (NEW)
                       ...
                       Plus 8 modules promoted from L4 (TransactionAwaitingCard, ...).
                       May import L0-L2. No service/store.

[L4] feature modules   src/popup/components/modules/  (existing, reduced to 11 SFCs after promotions)
...
[C1] service hooks     src/composables/  (flat) OR src/composables/services/  (subdir; requires vite.config.ts dirs update)
                       useFormState, useEntityCrud<T>, useFeeEstimation, useDappPayload, ...
...
**Entry**:
- M6 plan approved.
- **Brutalist redesign branch state confirmed with user**: if Input variant decision is pending elsewhere (`brutalist-redesign/post-rebrand` is master HEAD, redesign actively in flight), defer Phase 4b until that resolves.
...
2. **Hand-audit** the inventory:
   ...
   - 8 pure modules → tag for Phase 5 re-classification (L4 → L3).
...
4. **Write `implementations-plan/M6/decisions.md`** (NEW — moves Phase 4 decisions upfront):
   - Button variant naming proposal (e.g., `primary` / `secondary` / `outline` / `ghost` / `cta` × `small` / `medium` / `large`). User approves before Phase 4 starts.
   - Input variant resolution: drop `default`, keep brutalist as default? Or rename `Legacy`?
   - PopupCard refactor strategy: composable extraction vs L4 relocation.
   - User signs off; `decisions.md` becomes binding.
...
2. Create `vitest.components.config.ts`:
   - Extends `vitest.config.ts`
   - Override `include` to `**/*.test.ts` (Vue component tests live next to their SFCs)
   - Reuse jsdom env
...
### Phase 2 — Visual sandbox (Histoire) + visual regression (Lost Pixel) (~5-6 hr)

**Entry**: Phase 1 done.

**Work**:
1. Add devDeps:
   - `histoire` (pin to non-alpha — npm `latest` is alpha; use `0.17.x` line or current stable)
   - `@histoire/plugin-vue`
   - `lost-pixel` (visual regression)
2. Add `histoire.config.ts`, `bun run story` (dev), `bun run story:build` (CI).
3. Pilot: `components/ui/Button.story.vue` — full variant matrix.
4. **Hard decision gate** at end of pilot:
   - Does `bun run story:build` complete in <30s?
   - Does HMR work with auto-imported components?
   - Does `<style module>` render correctly in stories?
   - **If any answer is no**: switch to **Storybook 10 Vue3-Vite** (10 was October 2025; ESM-only; full Vue 3 support). NOT a custom playground page.
5. Wire **Lost Pixel** against `bun run story:build` output:
   - Configure baseline directory.
   - Initial baseline = post-redesign master.
   - CI gate: any diff > tolerance fails.
6. Update `conventions.md` with story conventions.

**Exit gate**:
- `bun run story:build` clean.
- `bun run lost-pixel` clean (initial baseline captured).
- One example story renders all Button variants.
- User verifies `bun run story` works locally.
- Hard decision gate passed (Histoire kept) OR Storybook 10 swap committed.
...
- **Migrate 4 native `<input>` sites** (`auth.vue:179`, `send.vue:458`, `AmountCard.vue:79`, `capabilities/index.vue:452`).
...
**4d — Other primitives** (in batches by domain similarity, NOT one-per-PR):
- **Layout batch**: Spinner, Badge, Banner
- **Form batch**: Toggle, Checkbox
- **Overlay batch**: Popover, Tooltip
- **Composite-ish batch**: Dropdown (DropdownRoot/Trigger/Item/Title/Divider), LoadingState
...
**Sub-PRs** (re-ordered per audit; `useFormState` ships from Phase 6 INTERLEAVED):
...
**5b — Re-classify pure modules to L3** (NEW from audit)
- Move 8 pure modules from `popup/components/modules/` to `src/components/composite/`:
  1. `TransactionAwaitingCard.vue` → `composite/activity/`
  2. `TransactionsList.vue` → `composite/activity/`
  3. `CapabilityDetailPanel.vue` → `composite/capabilities/`
  4. `EmojiGrid.vue` → `composite/general/`
  5. `WarningView.vue` → `composite/general/`
  6. `AmountCard.vue` → `composite/send/`
  7. `FeeJuiceCard.vue` → `composite/send/`
  8. `SendTypesCard.vue` → `composite/send/`
- Update import paths in all callers (rg + sed).
- Add stories + tests for each (≥5 cases each).
- Auto-import handles new paths transparently.

**INTERLUDE: 6d — `useFormState` ships here** (audit-driven re-order; was Phase 6 in v1)
- Controlled inputs + validation rules + dirty tracking.
- Tests ≥10 cases.
- Migrate `NewContactPopup` to use it.

**5c — `EntityForm<T>`** for NewX/EditX twins
- Built ON TOP of `useFormState`.
- Single-source field definitions; same form renders for new + edit.
- Migrate Contact pair (NewContact + EditContact → ContactForm) — saves ~150 lines per pair × 7 pairs = ~1050 lines.
- Roadmap: Account pair, Token pair, Network pair, Fpc pair, Endpoint pair migrate in follow-up PRs.

**5d — `SecretRevealCard`** — `key.vue` + `seed.vue` ONLY (NOT `full.vue`)
- Reveal-on-click, copy.
- Migrate one export page first; lines saved across 2 pages: ~250.
- `full.vue` decomposes separately in 7m.

**5e — `InputWithButton`** (recipient + scan/paste, password + show/hide)
- Story matrix.
- Migrate `send.vue` recipient field, all password fields.

**~~5f — ConfirmDialog upgrade~~** ❌ **DEFERRED** out of M6 per audit (scope creep; current `cacheStore.confirm.callback` pattern works).
...
Order of attack (composable-driven dependencies first):

| Sub-PR | Target | Current | Goal | Required composables |
|---|---|---|---|---|
...
| 7e | `send.vue` | 753 | ≤ 500 | useFeeEstimation, useFormState |
| 7f | `auth.vue` | 492 | ≤ 350 | useFormState |
| 7g | export pages (`key.vue` + `seed.vue`) | 644+577 | ≤ 250 each | (collapses into SecretRevealCard) |
...
| 7l | `discover/index.vue` | 448 | ≤ 350 | useDappInteractionPayload |
...
1. Add `biome.json` `noRestrictedImports` overrides:
   ...
   - **L4 modules** (the 11 remaining `popup/components/modules/**`) cannot import L5/L6.
...
4. **Final Lost Pixel run**: zero diffs vs. baseline (or explicit explanations).
...
- Next unchecked task: 4a complete the Button hover state baseline
...
- [x] @vue/test-utils + @pinia/testing installed (jsdom reused)
- [x] vitest.components.config.ts
...
- [x] histoire installed (pinned to 0.17.17)
- [x] histoire.config.ts
- [x] bun run story works
- [x] bun run story:build clean (15s)
- [x] Button.story.vue pilot
- [x] lost-pixel installed
- [x] Initial baseline captured
...
- [ ] 4b — Input (variant decision + 23 brutalist + 4 native)
- [ ] 4c — PopupCard refactor (useFullscreenPopupSetting)
- [ ] 4d — batches (Layout / Form / Overlay / Composite-ish / Settings)
...
| Phase | v1 estimate | v2 estimate (post-audit) |
|---|---|---|
| 0 — Discovery + decisions | 3-4 | 3-4 |
| 1 — Test infra | 3-4 | 3-4 |
| 2 — Histoire + Lost Pixel | 3-4 | **5-6** (added LP setup) |
| 3 — Tokens | 2 | 2 |
| 4 — Primitives | 6-8 | **7-9** (added PopupCard refactor) |
| 5 — Composites + re-classify | 6-8 | **7-9** (added module re-classification, useFormState interleave) |
| 6 — Composables (remainder) | 4-5 | **3-4** (useFormState moved to Phase 5) |
| 7 — Decomposition | 8-12 | **10-14** (added 7m for full.vue) |
| 8 — Layer enforcement | 2-3 | 2-3 |
| 9 — Docs | 2 | 2 |
| 10 — Final verification | 2-3 | 2-3 |
| **Total** | **41-55 hr** | **45-65 hr** |
...
| **Brutalist redesign half-applied** during arc | **H** | Phase 0 entry condition: confirm with user; Phase 4b decision absorbed in `decisions.md` |
| Visual fidelity loss | H | Lost Pixel automated diff per sub-PR (Phase 2 onward); manual smoke matrix in Phase 10 |
| Time cost (45-65 hr) | H | Phases independent; STATUS.md tracker enables pause/resume |
...
- [ ] **Plan scope** — M6 (renamed from A12) replaces A11; broader. OK?
- [ ] **Histoire** as visual sandbox with Storybook 10 fallback — OK?
- [ ] **Phase ordering** — `useFormState` interleaves into Phase 5 timeframe (between 5a and 5b). OK?
- [ ] **Time budget** — 45-65 hours over multiple sessions. OK?
- [ ] **Decisions absorbed in Phase 0** — Button variant naming + Input variant resolution + PopupCard strategy decided up-front (in `decisions.md`), not mid-arc. OK?
- [ ] **`SecretRevealCard` scope** — `key.vue` + `seed.vue` only; `full.vue` decomposes separately in Phase 7m. OK?
- [ ] **`ConfirmDialog` deferred** out of M6. OK?
- [ ] **Brutalist redesign coordination** — Phase 0 entry condition. OK?

## Audit divergences (between Codex + Claude)

Both audits converged on most findings. Differences:
- **Module re-classification**: Codex listed 3 (CapabilityDetailPanel, TransactionAwaitingCard, SendTypesCard); Claude listed 8 (verified via service/store import scan). v2 uses Claude's list.
- **Counts**: Codex didn't recount; Claude verified — 14 New/Edit pairs (not 24), 22 cta refs (not 23), 4 native inputs, 127 of 137 SFCs use module CSS (not 94). v2 uses Claude's verified counts.
- **`PopupCard` violation**: only Claude flagged. v2 absorbs as Phase 4c.
- **`type` prop rename surface**: only Claude flagged (64 callers). v2 absorbs in Phase 4a.
- **A12 → M6 rename**: only Claude flagged (`AUDIT.md:102` collision). v2 absorbs.
