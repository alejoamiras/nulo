...
| Browser-driven e2e for same-seed deletion containment (two same-seed profiles → delete one → survivor's UI intact) | Covered deterministically at unit/coordinator level (`account/composite-key.test.ts`, `transaction/service.test.ts:82`, `profile-deletion/coordinator.test.ts`); the import flow's multi-minute recovery envelope makes the e2e variant hostile (see the retired smoke test's replacement note in `tests/e2e/import-paths.test.ts`), and the UI deletion flow is a thin shell over the tested coordinator. |
