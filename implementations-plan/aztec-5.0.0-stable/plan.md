...
**Status: DRAFT v4 — fable r1 folded; codex r1 folded; final-pass r1 (reject, 7 blockers) folded with two explicit reasoned rejections; final-pass re-verdict pending on this v4.**
...
- PXE-store at-rest encryption (D9 — now its OWN gate question; default defer with exposure documented).
...
1. Per-(profile, chain) store injection (Nulo-owned names; `ChainRuntime` owns handles; fail-closed close/delete; the registry records backend type + identifier per store).
2. Purge rework (`store.delete()` + registry-driven removal; works with NO live runtime; `chain-coordinates.ts` comment updated).
3. Vite plumbing for `@aztec/sqlite3mc-wasm`.
4. **Committed spike script** against the PRODUCTION build: OPFS boot → `sqlite3.wasm` asset/MIME/CSP → create account → restart persistence → 2 profiles × 2 chains concurrent (no SAH contention) → targeted chain purge + full profile purge with negative control (B intact) → no-runtime purge via the registry → zero residual IndexedDB AND OPFS artifacts.
...
1. **Pre-flight — write + review `intent.json`**: node identity via the primary RPC, a second independent Aztec endpoint if one exists (else the documented single-node posture — Scope #10), L1 Registry/Rollup anchored via `cast`; rollupVersion `1821665230` re-confirmed (moved ⇒ STOP); signer = plan-pinned `0xFcc22383…` (env-derived address must match); every flag + amount + cap explicit; deployer envs present (never created/printed).
2. ChainId cascade (4 sites) + L1 constants re-pin.
3. **L1 fuel** via the wrapper: `SEED_AZLO_WETH=false SEED_ETH_FJ=false`, dry-run → review vs intent → `--broadcast --slow`. Re-validate node identity before the broadcast group.
4. **L2 bridge candidate-first** (+ faucet in parallel); candidate written under the strict schema; **candidate SHA-256 recorded into `intent.json`**.
5. **PrivateFPC**: hardened gate (exact version + digest + live class) → deploy at canonical salt `0x…01` (asserted `0x257aa870…`).
6. **SponsoredFPC**: expect the accelerator's funded instance; mismatch = stop.
7. **Pool seed** for the fresh AZLO via the wrapper (dry-run first; explicit amounts).
8. **Candidate proofs — ALL pre-promotion**: `verify-l1 --config` (supplementary) · `smoke-existing-testnet --config` · `smoke-swap-existing-testnet --config` (fueled) · `fuel-testnet.ts --config <candidate> PRIVATE_RUNS=1` (private settle) · **the direct Fee-Juice deposit→claim canary against the candidate's `l1.feeJuice`** (final-pass F4). Re-validate node identity before this group.
9. **Promote**: verify-intent re-run + candidate digest unchanged → copy → re-pin consumers atomically in one commit.
...
4. Follow-ups filed: exclude removal; D9 encryption; `minFuelFj` calibration if suggested; upgraded-contract support epic if the divergence helper ever fires.
...
4. **Plaintext-at-rest PXE store** — default: accept for this cycle (exposure documented); encryption follow-up on the injection seam (D9).
...
| D9 | At-rest encryption | DEFER with the exposure DOCUMENTED as its own gate decision | Adopt `openEncryptedStore` now | Bounded blast radius; injection seam is the hook; cheapest-adoption-point argument noted (final-pass) — user decides at the gate |
...
- **Codex final pass, re-verdict on v4:** _pending._
