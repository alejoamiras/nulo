...
| 4 (IN PROGRESS — 2/6 phases merged) | **Q10** composition-root migration | deep | dual-model reconciled (`lessons/q10.md`): inject runtime-owned storage port + PXE factory (always-inject, no fallback). **Storage-first, ~6 network-gated phases.** **P1 ✓** account/dapp-session/transaction (squash `28a7c24`, PR #152; full gate green + codex SHIP). **P2 ✓** auth-registry + balance/incoming-transfer repos (squash `378d0da`, PR #153; full gate green + codex SHIP; 4 tests → FakeBrowserApi). **P3 ✓** token/fpc/network + network's 3 raw active-key calls (squash `1a19be5`, PR #154; full gate green + codex SHIP). **STORAGE ARC COMPLETE (P1-P3).** Re-scoped closeout (1 PR): ProfileService is already port-aware (`browserApi?` optional) → just pass it at runtime.ts:136 (1 line); PXE = a non-breaking defaulted `PxeClientFactory` across the 8 `new PxeServiceClient` sites + runtime passes a runtime-owned factory (zero behavior change, tests untouched). Dead `chrome.storage` fallback branches left as the opportunistic cleanup codex flagged. Keys byte-identical (network-e2e backstop). |
