...
| 3 (re-verified LIVE, next) | **Q11** WalletSdkDispatcher | deep | **LIVE** — dispatcher 1011→1226 lines; the `getSessionAccountAddresses → caip + alias` projection is still reimplemented inline (alias projection at 943-944 dups `formatSessionAccounts` 438-439; single-account caip projections at 502/528, 593/599, 636/641). Route through one shared projection helper. **CONSTRAINTS: `formatSessionAccounts` wire-format parity pinned by dispatcher unit tests; the 3-copy `nulo-schema-patch` shape pinned by `dispatcher.test.ts` — preserve both.** Dual-model blueprint next. |
| 4 (re-verified LIVE+WIDE) | **Q10** composition-root migration | deep | **LIVE** — half-migration persists: 8 per-service `new PxeServiceClient` (account-state/execution/fpc/network/note/token/transaction/token-balance) + ~12 hard-coded `chrome.storage.local` (account/auth-registry/contact/dapp-session/fpc/network/operation-journal/token/profile/transaction + repos); `runtime.ts:107-109` comment confirms only ContactService is port-injected. Biggest remaining arc → incremental per-collaborator port injection (storage-first then PXE, per-phase network-gated); NOT a broad rewrite. Likely sub-phased. |
