...
| 2 (re-scoping) | **Q5** duplicated lifecycle tail | re-verify → **SHRUNK** | #83's `ExecutionLane` owns slot/journal/controller. Residual = whether the prove→submit→persist tail + `JobCancelledSentinel` catch is still duplicated across transfer-executor (×1) + dapp-send-executor (×3). Investigating. |
