...
> **✓ COMPLETE.** Design primitives + 20 extension sites tokenized; 3 ghosts caught & fixed (`--nulo-primary`, `--nulo-error` ×2 forms — guard hardened to prop form); FOUC fixed via external `theme-boot.js` + localStorage (build-verified). Gate: design 265/4xfail/1skip, extension **2598 pass**, typecheck + lint + build green; grep gate clean (dark scrims survive by design). See `lessons/phase-1.md`.
...
> **✓ COMPLETE.** Direction A (accent `#a8480c`) — **pre-approved by the user at the blueprint gate** ("Approved as is. Palette direction: (A)") — landed in `[theme="light"]` + explicit `--border` + `color-scheme`. Contrast gate **GREEN both themes** (18/18); base.css SHA updated; dark frozen (diff = only additive `color-scheme: dark`); design 270 / faucet 413 / build-storybook green. Rendered palette surfaced for visual review (Storybook theme toolbar) + revertible in one commit; the user can request value tweaks anytime (the gate re-verifies). See `lessons/phase-2.md`.
...
> **✓ COMPLETE.** `useTheme` (allowlist-validated) + `ThemeToggle` in the App.vue topbar + external `theme-boot.js` FOUC fix (build-verified) + 6 ghost vars fixed. Gate: faucet **422 pass** (+9), typecheck + lint + build green, undefined-var guard GREEN. See `lessons/phase-3.md`.
...
> **✓ COMPLETE.** `audit:vue` GREEN (typecheck:all + extension 2598 + lint + build); design gate GREEN (270, run explicitly per H1); build-storybook GREEN; **`CI=true bun run test:e2e` → `TEST_E2E_EXIT_CODE=0`** (69 passed / 7 skipped) — the gate passes the way CI evaluates it. The one local failure was the repo's OWN `test.skipIf(process.env.CI)` passkey-backup test (`:173`, deliberately CI-skipped — times out on hosted CI, kept local-only; also fails identically on clean `dev`, proving it's not this work; smoke is advisory per CLAUDE.md). Codex `019efd42` found the skip. Manual security/affordance smoke handed to the user. See `lessons/phase-4.md`.
...
- **Codex post-impl audit:** **BLOCK** — 2 HIGH (both real, both folded):
  - **HIGH-1:** light `--txt-body` (3.36:1) + `--txt-tertiary` (2.43:1) carry SECURITY copy (scam-token trust prompt, "irreversible" confirm, reset guidance) below AA. Root-caused: the DARK muted tokens are ALSO sub-AA (~3.8/2.5) — a pre-existing, frozen, de-emphasized scale, NOT introduced here. Fixed the NEW (light) theme: raised light muted tokens to AA on every surface (`--txt-body 58%`, `--txt-tertiary 56%`, `--txt-support 56%`); dark `rgba(245,240,230,…)` muted UNCHANGED (verified by diff); SHA re-pinned.
  - **HIGH-2:** the gate never asserted the muted pairs (only primary/secondary/inverse) → HIGH-1 shipped green. Fixed: added a required light suite asserting `--txt-body/tertiary/support` ≥4.5 on `--card-bg`/`--nulo-surface`/`-low`/`-high` (gate 31/31 green). Dark muted NOT asserted (frozen, pre-existing sub-AA = a separate, surfaced finding).
  - Gates after fix: design 283, extension 2598. **Codex re-audit (resume): `ship`** — both HIGH cleared (verified live ratios 5.33/4.99/4.95/4.63, all AA; gate guards them); dark muted = agreed pre-existing debt, separate follow-up, not a blocker.
  - **PRE-EXISTING finding surfaced to the user:** the DARK theme's muted-text scale (`body`/`tertiary`/`support`) is also below WCAG-AA — a shipped, frozen baseline this work did not change. Worth a follow-up to raise dark muted contrast too (out of this plan's "dark frozen" scope).
