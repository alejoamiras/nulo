...
> **Status: DRAFT v2 (blueprint mid, awaiting approval).** Successor to [`proverless-e2e-diagnosis`](../proverless-e2e-diagnosis/DIAGNOSIS.md). **v1 (a test-only PXE-readiness gate) was REJECTED by both auditors** — the signal isn't test-readable, it's decoupled from the real backpressure, and placement couldn't cover the stall (see `audit-codex.md` / `audit-fable.md`). **User then approved a minimal production fix.** This v2 cuts the backpressure at its source.
