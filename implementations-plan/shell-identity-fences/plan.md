...
- Implementation deviations (reviewer-ratified at the max review): (1) `awaitProfileActivation` is a SINGLE watcher composing the bounded wait + identity match + bootstrapFailure join — not the specced `Promise.race` of separate waiters, whose losing branch would leak its store watcher for up to the full bound on every unlock; (2) auth.vue's `BootstrapFailedError` branch releases silently with NO auth-side toast — the shell's `onActiveProfileChanged` catch already toasts the same failure, and double-toasting the one event was rejected. Both preserve the ledger's adjudicated semantics (immediate release on definitive rejection; discriminated messaging).
