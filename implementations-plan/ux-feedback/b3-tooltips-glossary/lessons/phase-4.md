...
## After the gate: the owner's U8/U9 sign-off (2026-09-25)

The owner answered round 5's T group in chat: "And on "T" looks good. But please, drop the usage
of "em dashes"." Both texts stay as drawn, and each sentence's em dash becomes a full stop
(`e1504b97`): "Verify carefully. Some characters can imitate Latin letters." and "Use a phrase
generated by Nulo. A phrase shared with another wallet lets that software derive your keys."

- The two component tests that pin the strings were changed first and shown red (2 failed,
  16 passed), then green with the components (18 passed).
- `gen_r5.py` and its regenerated `src/r5/tips.html` carry the same text; regenerating changed no
  other round-5 part. The spec's U8 and U9 rows say it too, and U9's row now places the line
  between the label and the field, as drawn, which closes the spec-wording ask.
- No e2e spec reads `dapp-hostname-warning` or `import-seed-note` (grep of `tests/e2e`), so the
  smoke and network suites are unaffected; the stack-top gates run both browsers in full.

| Gate on `e1504b97` | Result |
|---|---|
| `bun run lint` | exit 0 (2 s) |
| `bun run typecheck:all` | exit 0 (42 s) |
| `bun run test:all` | exit 0 (130 s) |
| `bun run test:ci-gating` | exit 0 (28 s) |
| `bun run build` | exit 0 (49 s) |
| `bun run --cwd apps/extension build-storybook` | exit 0 (27 s) |
