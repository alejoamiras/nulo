...
| L2 | **Budget: no top-up required by default.** The live ETH/FJ pool exists; we seed only AZLO/WETH (~0.068 WETH + free AZLO at L=1e19) ≈ 0.08 ETH total spend vs 0.168 held. `SEED_ETH_FJ=false` default; P1 fork-probes the live pool's usability and gates the decision with evidence. Codex's "thin pools are a security bug, top up" concern is real but bounded here: drain bounty ≈ the 0.068 WETH seed, failure mode is availability-only GIVEN the L1 hardening lands (with both requires, hostile-target paths revert; pre-fix the residue path was theft — codex assumption-attack). Top-up survives as an Ask for re-seed headroom. | fable D4 over codex P2 / main P2 | **ADOPTED** (Ask 1 confirms) |
...
| L8 | **Fuel UX = exact-input AZLO slice with quoted FJ estimate**, never "target FJ output" (inverse quoting = more failure modes). Quote at submit; REFUSE to proceed without a quote (`minFuelOutput` of 0/1 signs away the slice); `MIN_FUEL_FJ` floor calibrated in P5 against a real claim fee (L15 ordering). | codex + fable D9 | **ADOPTED** |
...
| L10 | **Pool parameters**: 10,000 AZLO/WETH mirroring the live pool's 10,000 FJ/ETH → 1 AZLO ≈ 1 FJ through the route (legible UX: "10 AZLO ≈ 9.9 FJ"). AZLO < WETH by address → currency0 = AZLO (asserted at runtime); both 18-dec; `AZLO_WETH_SQRT_PRICE = floor(2^96/100)`, band mirrors the ETH/FJ band (`-115140..-69060`, init tick ≈ −92109). ALL derived constants are fork-test-gated, never trusted bare. | fable D3 | **ADOPTED** (Ask 2 confirms rate) |
...
| L14 | **Ladder reachability — POSITIVE-EVIDENCE TRIGGERS ONLY** (v2; the v1 differential-simulation probe was rejected by BOTH double audits: the repo's claim classifier deliberately conflates "consumed" with "PXE not synced", and the private variant compares different sync paths). The sponsored ladder fires only on ground truth: (a) the journal-first `fuel.claimAttempt` + persisted `claimTxHash` whose receipt reads INCLUDED ⇒ `consumed=true` (closes the crash window); (b) a public-FJ BALANCE probe — `balance_of_public(FeeJuice, user) >= fuel.received` while the token is unclaimed ⇒ fuel already landed (closes third-party-trigger and fee-spike paths; the FJ is recipient-bound so "landed" is monotone truth). NEVER infer from simulate-failure; no positive evidence = keep waiting (the safe default — kills the premature-fallback risk codex raised: sponsored fallback can no longer orphan an unclaimed FJ message). The fallback is non-destructive by construction: fuel is never marked abandoned. Needs `balance_of_public` on the FJ contract in the UTILITY simulation scope (P4). | fable CC + codex & fable double audits | **ADOPTED (v2)** |
...
- Broadcast `DeployFuelLive.s.sol` (budget per L2: `WETH_SEED≈0.07`, `AZLO_WETH_LIQUIDITY=1e19`; `SEED_ETH_FJ` only if P1's probe demanded it). Deployer key from `packages/bridge-core/.env`, never printed.
...
- `capabilities.ts`: `{ contract: FEE_JUICE_L2 (feeJuiceAddress), function: "claim_and_end_setup" }` in `transaction.scope` AND `simulation.transactions.scope`, plus `balance_of_public` on FJ in the UTILITY simulation scope (the L14 balance probe); consent copy names the gas claim.
...
- Pure `lib/fuel-claim-state.ts` (codex modularity): `decideFuelClaim({ attempt, txReceiptStatus, fjBalance, fuelReceived, tokenClaimed }) → "fjwc" | "sponsored" | "wait"` — the L14 v2 ladder as a unit-pinned pure function (positive-evidence only), consumed by the engine.
...
**Gate:** `bun run audit:vue` green; component/unit pins REQUIRED: `decideFuelClaim` truth table (all three L14 stranded paths reach "sponsored"; no-evidence ⇒ "wait", never premature), fjwc payment selection for fuel records (Sponsored for non-fuel), floor read from config, toggle-off = byte-identical legacy behavior, quote-refusal blocks submit, private+fuel seals only the token secret, fuel phases derive correctly post-reload.
...
- **Owner-settable `setSwapTarget`** (headline trust knob): post-L1 hardening, a hostile target can no longer under-deliver below the user's SIGNED `minFuelOutput` — the router enforces the bound itself; balance-delta check prevents over-reporting. Residual: griefing by revert (DoS; atomic revert refunds), economically-pointless substitution. Owner = deploy EOA via `Ownable2Step` (no single-tx hijack); custody recorded in `deployments.md`. The witness binds `routeHash` — relayers can't re-route; only OWNER threatens the route, only within the signed bound. Mainnet would need timelock/multisig (recorded, out of scope).
...
- **Local persistence**: fuel secret plaintext like public deposit secrets — tampering/theft never redirects funds (the FJ content hash binds the recipient, L3). Precision from the contradiction-check: a LEAKED secret+leafIndex does let a third party TRIGGER the claim early (griefing, not theft) — the FJ still lands in the user's public balance, but the fjwc self-paying claim then fails; the L14 consumption probe detects exactly this and routes to the sponsored ladder. Backup stays strict: malformed fuel rejects restore.
...
- fjwc works when the tx's MAIN call is the token claim (the strategy was built for it; no dApp has exercised it) — P7 item 2 is the first real exercise; graceful degradation to the sponsored ladder is the documented contingency (codex: HIGH inference, the ladder rides on it).

**Asks** (user decisions at the approval gate — Ask "extend re-consent or manual fallback" was RESOLVED to firm in-arc work by the contradiction-check, L9):
1. **Budget**: accept no-top-up (seed ~0.07 ETH of the 0.168), or top the deployer to ~0.3-0.5 first for re-seed headroom after potential pool-drain griefing? (Recommended: no-top-up; re-seed is cheap if griefed.)
2. **Pool rate + fuel preset**: confirm 10,000 AZLO/WETH (→ 1 AZLO ≈ 1 FJ) and a prefilled editable fuel slice (e.g. 10 AZLO) as the UI default.
3. **Owner model**: SwapBridgeRouter owner stays the deploy EOA, vs a multisig now (not recommended on testnet). Be explicit about what the owner can do: set the swap target (bounded by BOTH L1 requires after hardening) and `sweep` residue/dust from router+swapper. Recorded in `deployments.md`.
