...
**Gate:** `bun run audit:vue` green; component/unit pins REQUIRED: `decideFuelClaim` truth table (crash-window via receipt, fee-spike via fee-insufficiency, leaked-secret edge via explicit user override only; no-evidence ⇒ "wait", never premature; a user already holding FJ changes NOTHING), fjwc payment selection for fuel records (Sponsored for non-fuel), floor read from config, toggle-off = byte-identical legacy behavior, quote-refusal blocks submit, private+fuel seals only the token secret, fuel phases derive correctly post-reload.
...
- Final gate re-verdict (same session, on v3): _pending_
