...
## Codex round 1 (Arc A): four medium findings, all adopted

Codex confirmed the migration's data itself: all 679 rows match the pre-A bytes, the promotions and renames are faithful, and the committed rewrite preserves every visible text and all 13,432 code spans and blocks. The four findings are gaps in verification and enforcement. Each fixture below failed before its fix.

| # | Finding | Fix | Fixture (before → after) |
|---|---|---|---|
| 1 | `untrack.ts --verify` never compared a row with the bytes actually being removed, so an upstream edit resolved as a deletion after a rebase leaves stale evidence that verifies. | `apply` refuses a path whose index blob is not its row's. `--verify` refuses any path the branch removes whose blob at the merge-base with `origin/dev` is not its row's. The nested ignore file, which `-M` pairs as a rename, now counts as removed too. (`f942eefe`) | `untrack.test.ts` "a path whose bytes in the base being untracked differ from its row is refused": an upstream edit lands on dev and the branch rebases onto it. Before: `apply` untracked the path and `verify` returned `[]`. After: `apply` throws and `verify` reports the drift. |
| 2 | The render-equivalence oracle ran a regex over HTML text, and the rewriter over the source, so the code span `` `href=audit-x.md ` `` was rewritten along with the real link and the proof accepted it. | The oracle parses both pages into nodes, substitutes real `href`/`src` attributes (unwrapping a de-linked `<a>`), and separately requires every text and code node unchanged. The rewriter tries each candidate alone and keeps it only if it leaves every text, code and comment node as it was and changes the page's tags. (`ebc40c96`) | `rewrite-links.test.ts` "code is never rewritten, and the oracle refuses a rewrite that changes a code node": codex's counterexample, a code span `[x](audit-x.md)` and a fenced `[y](audit-x.md)`. Before: both link and code span rewritten, and `prove` accepted the corrupted counterexample (probed). After: only the link changes, and `prove` throws. |
| 3 | `CSS_INERT_RE` ended strings only at LF, so `content:"x&#13;;background:u\72l(…);/*"` hid its escaped `url()` inside a string that CSS ends at the CR. | `cssUrls` applies CSS input preprocessing first (css-syntax-3 § 3.3: CR, CRLF, FF → LF; NUL → U+FFFD). (`eca153ae`) | `links.test.ts` "a CR or form feed ends a CSS string as a newline does": codex's probe (CR via `&#13;` in a `style` attribute), FF via `&#12;` in an attribute, a literal CR and a literal FF in `<style>` blocks. Before: 0 findings. After: 4 `link-opaque`. |
| 4 | `check.ts` ignored `mode()`: run directly on a push, nightly or release with an enforced finding, it exited 1 against the report-only policy. | The CLI exits by `verdict(findings, process.env)`; `--report` still always exits 0. (`acaa74c2`) | `tree.test.ts` "the CLI exits by the same verdict": `check.ts` spawned in a fixture repo holding a tracked transcript. Before: the push run exited 1. After: push exits 0 and prints the finding; a pull request and a local run exit 1; `--report` exits 0. |

**The committed rewrite stands.** `rewrite-links.ts --verify 0c95ac1b`, re-run under the new rewriter and oracle, re-derives all 48 files byte-identically: 0 problems. That matches codex's independent inspection. The tree's report is unchanged too: 5 `path-token` findings, 0 enforced.

**Gate on `acaa74c2`** (all exit 0): `check.ts` 5 findings, 0 enforced; `untrack.ts --verify` 679 rows, 0 problems; `rewrite-links.ts --verify 0c95ac1b` 48 files, 0 problems; `bun test scripts/ci-cd/plans/` 94 pass; tool tests 8 pass; `test:ci-gating` 244 pass, 2 skip; `lint`, `lint:actions` and `check-no-local-paths.sh` clean; `bun run test` 7,068 pass; every tool rerun leaves `git status` empty; a fresh clone reports the same.
