...
**Gate** — `bun run --cwd apps/extension vitest run src/wallet/services/execution src/composables` then `bun run lint && bun run typecheck:all && bun run test`. Pass: cancel-before-pipeline, no-stash-on-cancel, foreign-token no-op, duplicate-rejection, evict-on-cancel-race, per-profile-cap (oldest-cancelled), and **Send handoff-race** (submit disarms unmount cancel; reuse entry survives navigation) tests green; full suite exit 0. Layers: typecheck/lint/unit.
