...
**Amended during implementation (2026-09-20) — three things the spike got wrong or missed; details in `lessons/phase-9.md`.**
...
3. The group's `test` excludes `@aztec/wallet-sdk`. With the plain `/node_modules/` test the content script's two web-accessible chunks (`crypto`, `handlers`) became three differently-named `vendor~content…` chunks — same modules, one more file readable by every page. With the exclusion both browsers' `web_accessible_resources` equal the pre-change lists exactly.
