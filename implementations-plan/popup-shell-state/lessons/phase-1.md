...
- **A sync branch beside an awaited one gets a sync-shaped helper.** `registerSenderForRow` returns
  the `addSender(...).then(ok, warn)` promise or `undefined` for the no-network skip, so the caller
  awaits only when a promise exists — the skip branch keeps its historical zero-await shape.
