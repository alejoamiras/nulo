...
4. **5.0 node API rename** — `node.isL1ToL2MessageSynced` (removed in 5.0) → `getL1ToL2MessageMembershipWitness("latest", hash)` in the Fee Juice claim poll (`fixtures/aztec.ts`); fixed `heavy/fee-methods`. Slipped past `typecheck:all` because `tests/e2e` isn't in the tsconfig `include`.
