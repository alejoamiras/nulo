...
**5.0 CI adaptation fixes (root-cause writeup in `lessons/phase-6.md`).** Five blockers across two classes — three **boot** blockers (systematic `exit 86`, before any test ran) and two that surfaced **once the suite executed** (5/5 bulk shards + heavy/concurrent-confirm passed):
1. **Bundled binary rename** — 5.0 renamed the bare foundry binaries on PATH (`anvil` → `aztec-anvil`). `global-setup.ts` probed the old name → anvil spawn failed. Fixed in `global-setup.ts` + `setup-aztec/action.yml` + `docker-ci-like.sh` (commit `a6fe038a`).
2. **Wrapper bypass → wrong `forge` for L1 deploy** — `global-setup.ts` spawns `node_modules/.bin/aztec` directly, skipping the `bin/aztec` wrapper that prepends `internal-bin` (the version-matched bundled `forge`/`cast`/`anvil`) to PATH. The node's L1 deploy then used the system `forge`: fine locally (foundry 1.4.1 matches the bundle) but CI's `foundry-toolchain@v1` installs latest stable → `deploy_aztec_l1_contracts: required arguments were not provided` → exit 2. Fixed by prepending `internal-bin` to the node spawn's PATH env (commit `452ae853`). Proven locally — stubbing a wrong-version `forge` reproduces the CI failure without the prepend and boots healthy with it.
3. **Hardcoded e2e fee ceiling fell below 5.0's base fee** — once the node booted, fixture-contract deploy failed: `maxFeesPerGas.feePerL2Gas must be >= gasFees.feePerL2Gas` (cap=1e11 vs live ≈9.24e11). 5.0 raised the L2 base fee ~4 orders of magnitude; `fixtures/aztec.ts`'s flat `E2E_FEE_GAS` cap (generous in 4.2.0) was now too low. Bumped 1e11 → 1e13 (~11× headroom over live, still under the SponsoredFPC fee-juice balance). The wallet's *runtime* fee path was already 5.0-safe (derives from `node.getCurrentMinFees()`); only the flat fixture cap needed it.
4. **5.0 node API rename** — `node.isL1ToL2MessageSynced` (removed in 5.0) → `getL1ToL2MessageCheckpoint(hash)` in the Fee Juice claim poll (`fixtures/aztec.ts`); fixed `heavy/fee-methods`. (First tried `getL1ToL2MessageMembershipWitness("latest", hash)` — it computes a claim proof and *hangs* when the message isn't in the tree yet; the checkpoint query is the right hash-only presence poll.) Slipped past `typecheck:all` because `tests/e2e` isn't in the tsconfig `include`.
6. **Native-proving toast timeout (transfers canary)** — `sendTransfer`'s 60s "Transaction submitted" wait is too tight for the prover-ON canary (the toast follows client-side proving; the shield path exceeds 60s). Made prover-aware (`NULO_E2E_PROVERLESS` → 60s vs 300s). Proverless bulk shards keep honest-fast failures; the canary matches `tx-sendTx-default`'s 300s.
7. **Missing `senderForTags` → all private-note txs (shield, private transfers) fail in witness-gen (CRITICAL — real-user correctness fix, not test-only).** 5.0's private-log delivery computes a discovery tag requiring `senderForTags`; the SDK's `EmbeddedWallet` injects it on every `simulateTx`/`proveTx`, but we call `pxe.proveTx`/`simulateTx` **directly** (`packages/aztec-runtime/src/pxe/service.ts`), so public→private shield + every private-note-emitting tx threw `"Sender for tags is not set"` before proving (public→public unaffected — no private note). Fixed: `senderForTags: scopes[0]` (the tx sender, by our `[account.address, ...]` scopes convention) on both calls. Found via **3 parallel codex agents** (5.0 / Wonderland token / our wallet — they disagreed; reconciled against `base_wallet`'s exact call shape). Verified locally (proverless `transfers.test.ts` scenario green end-to-end). See lessons/phase-6.md Iteration 4.
8. **Fee Juice claim not claimable — 5.0 mints no empty blocks.** The bridged FJ claim threw `No L1 to L2 message found`: 5.0 only builds an L2 block when txs are pending, so the anchor stalled below the message's checkpoint and the claim's membership witness couldn't be built. `waitForL1ToL2Message` now waits on the real predicate (anchor checkpoint ≥ message checkpoint) and drives blocks via a caller-supplied `forceBlock` (a cheap sponsored mint). `node.mineBlock()` is interface-only (not in the admin RPC schema). Fixed `fee-methods` public-FJ path.
9. **PrivateFPC registration — 5.0 moved deploy options to construction time.** Private-FJ setup threw `deployer is not yet locked` from `PrivateFPCContract.deploy().register({deployer: ZERO})`. Fixed by computing+registering the instance the wallet's way — `getContractInstanceFromInstantiationParams({salt: ZERO, deployer: ZERO})` + `registerContract` + `.at` — which sidesteps the locked-deployer error AND guarantees the address matches `fpc/service.ts` auto-discovery. Fixed `fee-methods` private-FJ path. All 5 `fee-methods` tests pass proverless locally. See lessons/phase-6.md Iteration 5.
5. **Accelerator origin gating (Phase-0 pin regression)** — accelerator-server v1.0.6 (SEC-01c) made unset `ALLOWED_ORIGINS` **deny-by-default**, denying the offscreen prover's `chrome-extension://` `/prove` in headless CI (no popup) → canary tx errors/timeouts despite `bb_available=true`. Set `ACCEL_ALLOW_ALL=1` (`_network-e2e.yml`), restoring the documented headless approve-all; SECURITY.md updated. Fixed `canary/real-proving`.
