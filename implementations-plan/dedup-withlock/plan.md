...
1. `Lock` (`packages/wallet-core/src/utils/lock.ts`) is a FIFO mutex; its internal promise only resolves; PRE-HARDENING, the only theoretical rejection paths in `enter()` were a throwing `ILogger.log` (pre-enqueue "waiting" log, post-acquisition "acquired" log) or a throwing `setTimeout` — a post-acquisition throw rejected AFTER ownership transferred with the force-release timer not yet armed (lock stranded). R2 closed all of these. Only 4 production Lock constructions carry a logger.
...
- I1 (revised): with R2 in place, "`await enter()` resolved" ≡ "ownership transferred" by construction (timer-arming is best-effort, outside the guarantee), so `withLock`'s `leave-iff-entered` contract is exactly correct at ALL sites regardless of their historical frame shape. Without R2 this equivalence is false (codex blocker) — R2 is what makes the mechanical migration sound.
...
- Hardening characterization: throwing logger at both throw points (incl. the >50ms contended acquired-log path) and throwing setTimeout → enter() still resolves, no waiter released spuriously, lock stays releasable (untimed on the setTimeout-throw path — accepted).
...
1b. ✅ `enter()` hardening + T1/T2/T3/T5 + characterization tests (implemented at HEAD; wallet-core 192/192).
...
- Mutual exclusion is the security surface; FIFO order and failure-recording-under-lock are preserved by recipes 1-6; timer-arming is best-effort under R2 (see §Hardening); R2 strictly narrows failure modes (no spurious release, no stranded lock).
