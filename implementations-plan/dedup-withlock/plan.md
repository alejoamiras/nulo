...
`enter()` becomes non-throwing by construction: the two logger calls and the timer-arm are wrapped so no logger/setTimeout throw can (a) reject `enter()` after ownership transfer, (b) skip arming the force-release timer, or (c) propagate into a hand-rolled frame's catch. Characterization tests pin: enter() resolves + timer armed even with a throwing logger (pre- and post-acquisition throw points). This also retires the latent "pre-enqueue logger throw → finally releases another holder" bug class in today's 52 enter-inside-try frames.
...
2. **Mechanical (~61 sites incl. config/store.ts)**: body moves into the closure; early `return x` inside the old try becomes `return x` inside the closure with the outer statement `return await lock.withLock(...)`. Hand-reviewed per site; watch fpc `getFpcs` (`fpc/service.ts:152-220`, fable M3: two DIFFERENT `return result.map(decorate)` sites straddling the release — the post-finally one must stay outside the closure).
...
4. **Promise-creation site (dapp-interaction:286)**: assign-out shape — the closure CREATES the popup promise and assigns it to an outer local, returning void; the method returns the captured promise after `withLock` resolves. Lock releases at promise creation, exactly as today.
