...
- **Initial (parallel):** **codex → reject**: the id-only fence is **ABA-unsafe** — an A→B→A round trip makes the parked run's `(profile.id, network.id)` capture match again, landing stale A2 over the fresh run's A1; demanded a monotonic activation epoch + an A→B→A regression test. **fable/opus → conditional approve** (condition: fix a factually-false vitest-defers claim in the collector comment); non-blocking: no-await-span fragility (NB-2), `commitScopeChange` conditional-commit trap for future callers (NB-6), the plan's promised composable-variant test not implemented (NB-3), same-scope `selectAccount` supersede out of fence scope (NB-5, pre-existing).
- **Round-1 fixes:** monotonic `activationEpoch` added as the load-bearing fence half (`superseded() = epoch mismatch OR id mismatch` — ids still catch a flip whose activation hasn't re-entered); **A→B→A regression test added with the newer activations RUNNING real `setupActiveAccount`** (direct assignment can't bump the epoch — codex's test critique), source-swap RED-verified against the id-only fence (stale `0xA2` landed). Doc fixes: corrected the vitest claim (fable's condition); no-await-span INVARIANT comment; `commitScopeChange` return-contract doc.
- **Documented deviations:** (NB-3) the plan's `useProfileBootstrap` park-inside-store variant is superseded by the store-level suite — the store tests park at the exact same await with the same observable assertions; the composable suite keeps its own fence pins. (NB-5) a same-scope supersede (`selectAccount` racing a parked run) is a PRE-EXISTING adjacent gap outside F-B27's proven counter-example — recorded for the owner, not silently fixed (bug arcs change behavior only per the proven fix).
- **Round-1 → codex resume: conditional approve (converged).** "No remaining behavioral blocker — the epoch closes the reported ABA … the ABA test is genuine" (verified RED against the id-only commit). Its one condition was doc-only and reversed fable's: the ORIGINAL "vitest defers to user listeners" claim matches the implementation (vitest's worker handler returns when another process listener exists — source-cited). Reconciled: vitest defers while the collector is installed (tests 1–2); fable's observed run-errors came from the collector-less queue-advance test, pre-fix only. Comment rewritten to state both halves precisely. **Both legs approve with conditions folded — converged.**
