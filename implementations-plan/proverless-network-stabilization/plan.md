...
**Validation gate:** `bun run lint && bun run typecheck` exit 0; `bun run --cwd packages/extension vitest run src/wallet/services/operation-journal/service.test.ts` green; `for i in $(seq 1 10); do NULO_E2E_PROVERLESS=1 bun run e2e:agent tests/e2e/network/<file> || break; done` → **ACHIEVED:** concurrent-sendtx-approve (the Mode-1 poster child) **6/6** green — proves journal-counting replaced the DOM race. The migrated observation assertions in concurrent-sendtx + concurrent-sendtx-confirm reach PAST the assertion to the settle (end-to-end green is gated on settle-layer Mode 3/4, owned by Phase 4). single-tx feePayer + multi-account-from green in a clean env. grep proves no helper conflates journal-state with card-render. ENV NOTE: leaked Vite servers degrade the Mac over long loops (pkill between runs; teardown fix in Phase 2). DEFERRED to Phase 4: concurrent-sendtx r2 reject-settle (Mode 3) + concurrent-sendtx-confirm T2 hang (Mode 4) — local repro captured in lessons/mode-4-local-repro.md. Layers: **TC · L · U · EP**.
...
**Validation gate:** `bun run lint && bun run typecheck && bun run --filter @nulo/extension test` exit 0; `git diff --stat -- packages/extension/src` shows zero `src/` changes (diagnostic is test-only); `lessons/phase-2.md` classifies all 4 modes. **ACHIEVED:** lint + typecheck exit 0; journal.ts e2e green (the dumpJournal/awaitOrDump instrument doesn't affect the success path); `network-e2e-soak.yml` built (here, not Phase 5) + actionlint-clean; `NULO_E2E_RETRY` parameterized; biome `!**/target` exclude fixes a pre-existing local lint break. DE-SCOPED: vite-teardown (low CI value — ephemeral runners; 2 attempts failed; pkill workaround). DEFERRED: e2e-typecheck gap. Layers: **TC · L · U**.
