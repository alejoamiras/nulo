...
**P8 — L2→L1 withdraw. 🟡 PROVEN (smoke)** (withdraw-public green end-to-end on the sandbox: `SetPublicAuthwitContractInteraction` burn-authwit → `exit_to_l1_public` → `waitForProven` → `computeL2ToL1MembershipWitness` → `portal.withdraw`; 40 USDC burned L2 → released L1. Remaining: through the app + localStorage resume + block-bar UI). P6 · HIGH. `burn_public` → `waitForBlockProven` (real blocks-remaining bar) → `computeWitness` → epoch via `getEpochForCheckpoint` → `Outbox.consume`. Point-of-no-return after burn. localStorage resume. **Validate:** real Aztec→Sepolia round-trip e2e.
