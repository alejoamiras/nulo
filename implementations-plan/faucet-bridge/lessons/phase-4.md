...
- ✅ `recovery.ts` — no-server resume schema: deposit + withdrawal records in two localStorage keys, via an injected `KV` interface (DOM-free → unit-testable). Claim secret held as an opaque `encryptedSecret` blob (R2: bearer credential for private transfers). `upsert`/`update`/`remove`/`find` + corrupt-JSON tolerance + bigint-as-string amounts. 9 unit tests (17 bridge-core total). typecheck + lint green.

## Next (authoring; pure parts unit-testable, node-touching parts integration-only)
- `recovery` ENCRYPTION layer — fill `encryptedSecret` by reusing `@nulo/wallet-crypto`'s PBKDF2+AES-GCM (R2: don't roll own) keyed off an **L1-signature-derived** key (R2: wallet-crypto's `EncryptionKey` is password-based → needs a sig-key wrapper). Note: wallet-crypto uses `self.crypto`, so its tests need jsdom/`self`.
