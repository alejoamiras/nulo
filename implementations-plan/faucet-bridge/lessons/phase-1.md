...
**Status:** IN PROGRESS. Live-net recon **BLOCKED** in this sandbox (needs operator infra); the compilable parts are proceeding.
...
## Doable next (live-net-independent)
- Strip `token_bridge` attestation + compile with rc.2 nargo.
- Keystone content-hash equality test (Solidity `Hash.sha256ToField` vs Noir `token_portal_content_hash_lib`) for fixed vectors.
