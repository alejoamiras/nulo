...
**Status:** ✅ **COMPLETE (2026-07-01) — `v0.24.0-rc.0` PUBLISHED** (prerelease GitHub Release + chrome/firefox zips + `SHASUMS256.txt`; Cloudflare deploys correctly skipped). All 3 phases done with **zero `--admin`** on any dev merge. Mechanism = **(2a) allow `["squash","merge"]` on dev** (chosen 2026-06-30 after Phase 1 proved bypass-actor-via-PR-merge infeasible). Phase 1 validated the full rc.0→rc.1→stable lifecycle in the throwaway; codex conditional-approve, all 5 findings adopted. Open follow-up: App-API-verified sync manifest commit (codex F1 clean fix; interim = `--merge --admin` on the forward sync).
