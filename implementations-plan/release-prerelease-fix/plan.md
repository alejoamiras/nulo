...
**Status:** ✅ **COMPLETE (2026-07-01) — `v0.24.0-rc.0` PUBLISHED** (prerelease GitHub Release + chrome/firefox zips + `SHASUMS256.txt`; Cloudflare deploys correctly skipped). All 3 phases done with **zero `--admin`** on any dev merge. Mechanism = **(2a) allow `["squash","merge"]` on dev** (chosen 2026-06-30 after Phase 1 proved bypass-actor-via-PR-merge infeasible). Phase 1 validated the full rc.0→rc.1→stable lifecycle in the throwaway; codex conditional-approve, all 5 findings adopted. **codex F1 follow-up DONE:** `open-sync-pr-run.ts` now writes the sync manifest re-baseline via the **Contents API under the App token** (GitHub-signed → verified), so the forward merge-sync needs **no `--admin`** (proven: `nulo-release-bot`'s API commits are PGP-verified; the old plain-`git-commit` was unsigned). First live exercise is the next stable release; `--admin` remains an emergency fallback only.
- **Phase 2 shipped:** PR #221 (rc.0 config + forward sync squash→merge) · dev ruleset += `merge` · PR #223 (commitlint-skip for sync PRs — see ledger) · PR #224 (one-time signed `-s ours` ancestry merge → `50b4145a` now a dev ancestor). #222 was the first ancestry attempt (closed — wrong branch name for the skip).
...
- **RESOLVED: dev non-squash-sync mechanism = (2a) allow `["squash","merge"]` on dev.** Phase 1 empirically KILLED the originally-recommended bypass-actor (1) AND admin `--merge` (3): a squash-only ruleset hard-blocks a PR merge-commit for plain-merge, `--admin`, AND a RepositoryRole-admin bypass actor alike (`lessons/phase-2.md`). Only allowing `"merge"` in the ruleset (or a bypass-actor DIRECT-push, not a PR merge) works. 2a chosen for simplicity (reuses the PR-based sync job).
- **NEW (Phase 2, real-repo only): the merge-sync PR trips commitlint** on main's immutable historical subjects (`base..head` carries the >100-char `release: promote …` commits). The throwaway never surfaced this (no commitlint gate). Fix = PR #223: extend the EXISTING `BASE_REF=main` commitlint-skip to sync PRs on dev (head `sync/main-to-dev*`) — narrow + consistent with the documented main-skip rationale; the one new sync commit (manifest re-baseline) is bot-conventional. This let the one-time ancestry merge (#224) land with **zero `--admin`**.
