...
**Status:** Phase 2 DONE (2026-07-01) — fix LIVE on `dev`, **zero `--admin`**. Phase 3 (publish `v0.24.0-rc.0`) in progress. Real-`dev` dry-run + the real prerelease workflow both now yield `chore(dev): release 0.24.0-rc.0` (bounded 47-commit changelog). Mechanism = **(2a) allow `["squash","merge"]` on dev** (chosen 2026-06-30 after Phase 1 proved bypass-actor-via-PR-merge infeasible). Phase 1 validated the full rc.0→rc.1→stable lifecycle in the throwaway; codex conditional-approve, all 5 findings adopted.
