...
- **Codex note on "architecture inventory":** `audit/**` and `implementations-plan/**` mentions are frozen historical run records — not edited (deviation documented). Codex's resume then pinpointed a genuinely LIVE inventory: `architecture/my-notes/02-service-inventory.md` still listed the method — removed.
- **Codex resume 1: `conditional approve`** — conditions: remove the stale service-inventory entry (DONE) + the in-flight SOLO e2e must pass (DONE before the resume returned: 2/2 green on the tightened predicates). All conditions met → **converged**.
