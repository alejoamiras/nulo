...
| D2 | prefixed raw-row codec (key, parse, scan, corrupt-id partition) | none shared; four classes (`TombstoneRepository`, `RestorePendingRepository`, `AccountIntegrityBlockedRepository`, `AccountIntegrityVerifiedStampRepository`) hand-roll it (searched `startsWith(prefix)`, `safeParse(JSON.parse`) | ~~build new: `RawPrefixedStore<T>` class~~ SUPERSEDED by the audit: two functions — `decodeRow(schema, raw)` (extension `wallet/utils/raw-row.ts`) and `prefixedEntries(all, prefix)` (wallet-core `storage/`, shared with A1); repositories keep their single-key reads and presence checks | 3 files, 4 classes |
...
| C2 | run a step under a task span | `startEstimateTask` (`execution/fee/fee-strategy.ts`) covers the start only | ~~adapt: `runTaskStep(task, fn)`~~ SKIPPED after audit: an `async` wrapper inserts a microtask before `task.complete()` (see plan.md § Decision ledger) | — |
...
| F3 | delete a balance row and invalidate it | none; ×4 (`invalidatedBalanceIds.add` ×4, one without an emit) | adapt: SYNCHRONOUS private `invalidateAndDelete(id)` returning the repo's own promise; loops keep their filters, fence and emit decisions | `token-balance/service.ts` |
...
| A1 | prefix scan of entity rows | none; ×5 scans (the prefix literal is built 11×) | adapt: `prefixedEntries(all, prefix)` in `wallet-core/src/storage/prefixed-entries.ts`, yielding `[key, id, value]`; shared with D2 | `wallet-core/src/storage/entity_storage.ts` |
...
| H2 | wait for profile activation | `awaitProfileActivation` (`composables/unlockWait.ts`, 3-signal) ⊃ `waitForProfileActive` (2-signal) | ~~reuse-as-is~~ SKIPPED after audit: the superset rejects at once on a matching `bootstrapFailure`, a failure-path timing change | — |
