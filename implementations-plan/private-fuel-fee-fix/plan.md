...
**Steps.**
- **Prerequisite (discovered Phase-2 investigation; scope addition — see lessons/phase-2.md): deploy the PrivateFPC on V5.** The PrivateFPC (`0x1fa8746e`, salt 0, 5.0 artifact `fb6f196` — address confirmed unchanged from V4) is NOT deployed on the V5 node (the SponsoredFPC is; the PrivateFPC's V4 deploy didn't carry to the fresh V5 rollup). The claim can't SETTLE without it on-chain (it's the fee payer), even though client-side simulation reaches its budget assert. Publish the class + deploy the instance at salt 0 on V5. **Held for user go-ahead (on-chain deploy + scope expansion).**
- Add a private variant to `fuel-testnet.ts` using `privateMintAndPayFee` (mirror the public block), registering the FPC via `getContractInstanceFromInstantiationParams(PrivateFPCArtifact, {salt: 0})` + `registerContract`.
