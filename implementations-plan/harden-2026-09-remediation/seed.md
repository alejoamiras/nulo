...
**Status:** complete — all four batches merged to dev 2026-09-15 via `gh stack merge` (stack #593: #591 audit docs + seed → `2bd32bb7`; B1 #592 → `f9ee51d6`, `b1-mechanical/plan.md`; B2 #594 → `bb479bc8`, `b2-backup-passkey/plan.md`; B3 #595 → `26e313eb`, `b3-isolation/plan.md`; B4 #596 → `ad130aea`, `b4-approval/plan.md`). Open owner actions: provision `passkey.nulo.sh` (B2), verify the Firefox sender shapes on a Firefox build (B3); F-09 lives in the Presto arc. Each batch below got its own `/blueprint` inside this arc, in order, driven by the `/goal` at the bottom.
