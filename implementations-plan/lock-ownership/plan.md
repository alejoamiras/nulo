# lock-ownership — batch 4 of audit-448-remediation (rev 3)
...
- **`session.set` rejection is indeterminate** (it may have written): on rejection the section performs a best-effort compensating `delete` of the row and never bumps. If the compensation also fails we are at today's pre-existing indeterminacy bound — but the unbumped generation means any pending close still runs its cleanup, and `restore()`'s expiry gate bounds a surviving fresh row to the open-failure UX, not a security hole. Specified, tested (rejection-after-write ordering).
...
- **Rejection-after-write**: `session.set` writes then rejects inside the section → compensating delete fires, generation never bumps, the pending close completes A's cleanup; assert no surviving row and no bump.
...
- Gate arc: round-1 dual audit (codex REJECT + fable APPROVE-WITH-CHANGES) → rev 2 → final fresh-context pass REJECT → rev 3 → resumed final-pass re-verdict (pending).
