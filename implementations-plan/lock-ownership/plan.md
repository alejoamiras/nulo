# lock-ownership — batch 4 of audit-448-remediation (rev 2)
...
- Private **artifact mutex** (`artifactLock = new Lock("session-artifacts", logger)`) — session-manager-INTERNAL, never held across facade-locked calls, so the runbook's reentrancy TRAP cannot fire.
- `open()`: crypto prep (wrapPair etc.) stays outside; the **artifact section** runs under the mutex: `{ sessionGeneration++; write row; set activeSession; schedule alarm }` — the bump is ATOMIC with B's artifacts landing. An open that fails before/inside the section without writing bumps nothing.
- `close(expected?)`: sync head gains an **identity guard** — when `expected` is passed (the `getActive` expiry path) and `this.activeSession !== expected`, return without doing anything (an off-lock expiry close racing a completed open must not nuke the successor's in-memory session/DEK). Head otherwise unchanged (zeroize, drop activeSession, emit — always refers to the session being closed). Then the **artifact section** under the mutex: `{ if (sessionGeneration !== genCapturedAtCloseEntry) stand down; else delete row; clear alarm }`.
- Why every interleaving is now correct: the legs serialize under the mutex, and a passing in-mutex re-check implies NO successor's artifacts exist yet (the bump is atomic with them); a failing re-check implies they ALL exist (stand down touches nothing). The codex failed-open hazard (A's bearer surviving its own close) is gone — a failed open never bumps, so the close proceeds and deletes A. Fable's close-captured-after-bump hole is gone — capture-after-bump implies B's section completed, and the head's identity guard (activeSession now B) stops the close before the artifact section; if the head ran pre-completion, the mutex serializes close's legs BEFORE open's (deleting A's row, clearing A's alarm — both still A's) and open's section then lands B's cleanly.
...
- Full stand-down ordering: B's open completes first → A's close head identity-guard (or in-mutex gen mismatch) stands down → B's artifacts untouched; assert also A's DEK zeroize still happened.
- Failed open: open throws before the artifact section → generation unbumped → a pending close still deletes A's row + clears A's alarm (no restorable bearer).
- Mid-open expiry close (fable's ordering): open parked at wrapPair (pre-section) → expiry close runs to completion (A's artifacts removed) → open resumes and lands B; assert B's alarm intact.
...
- **Adjudicated cross-auditor disagreement**: codex asked for re-checks before EVERY mutation/emit in the CS; fable's verified census shows all other awaits are storage/config-fast, every DESTRUCTIVE epoch-bumper holds the serviceLock (so mid-CS destruction requires watchdog revocation, whose only plausible >5-min park points are the two PXE awaits), and off-lock bumps come only from non-destructive `hydrateSchedulers`. The two-site placement covers the harm model; blanket checks are noise. Adopted fable's census; logged prominently (lessons + ledger).
- `serviceLock` keeps its default watchdog (both audits concur).
- Test (**the composed gold pin**, replacing rev 1's vacuous spec — `onTokenDeleted` queues on the same lock and can never bump while the CS is parked, so that harness passes green on revert): park the CS at a deferred `getBlockTimestamp` → advance fake timers past the serviceLock watchdog → the lock hands to the queued `onTokenDeleted` (bumps + wipes) → release the deferred → assert the revoked CS writes NOTHING (record, outbox row, trust, emits all absent). This composes N-11's handoff with N-17's re-check exactly as the production hazard does, and is RED on revert of the re-checks.
...
- Ticket over generation-number; handoff-mint; raw-API break (zero callers, verified twice); check-first `leave` order (fable — silently-revertible liveness); watchdog-own-ticket (runbook + both audits); network lock `maxHoldMs: null` (codex round 1, H1/H2 overlap guaranteed on by-design holds) vs serviceLock keeping its watchdog (fable verified).
- N-12: bare generation fence REJECTED by both audits (TOCTOU + capture-after-bump + failed-open bearer) → artifact mutex with bump-atomic-write + sync head identity guard. Alternatives D/E partially vindicated and subsumed.
- N-17: two-site placement upheld over codex's blanket ask via fable's mechanistic census — the one unresolved cross-auditor disagreement, adjudicated on verified evidence, logged.
- Test-vacuity findings (both audits, echoing batch 3): N-17 pin recomposed to the watchdog-handoff form; f1-1 adoption re-shaped to mutex-consistent assertions.
