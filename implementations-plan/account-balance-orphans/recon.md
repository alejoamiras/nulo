...
- **Dependency direction — CORRECTED after audit**: the draft claimed `AccountService` (phase 0) cannot reach `TokenBalanceService` (phase 1) and forced orchestration into the restore composable. Wrong: the repo's registration pattern (`registerChainPurgeSubscriber`, `account/service.ts:111` — AccountService registers an awaited `clearChainState` callback into NetworkService) is exactly a higher-phase service handing a lower-phase one an awaited callback with no import, no cycle, no event. `TokenBalanceService` registers its purge with `AccountService` the same way; `reconcileImportedAccounts` awaits it in-place, keeping the cascade internal (no RPC exposure) and inherited by any future account-removal path.
