...
| 4 `deflake-r2/cancel-fee` | A5: re-key BOTH helpers through ONE shared typed selector factored from `selectFeeMethod` (codex Medium 7 — it already selects by subtitle + gates on `data-fee-method`); union `"sponsored"|"public"|"private"`; exercise public+private via execute-flow select + assert + reject through the shared path. A4 (REVISED per codex Critical 2 — no honest e2e driver exists: `cancelInteraction` absent from RPC Methods/client, dispatcher passes `undefined` token, SDK aborts discovery only): take the PRODUCT fork with defense in depth (design per codex re-audit): cancellation becomes DURABLE — `cancelInteraction` stores `cancelled`/`cancelledAt` on the interaction record BEFORE emitting (an event-only cancel racing popup subscription is lost), and payload loading replays the state; semantics = **first service claim wins** (cancel processed first → later approve throws typed `JobCancelledError`, execution never starts; approve claimed first → later cancel finds nothing, approval proceeds exactly once — service acceptance is the commit point, not the browser click); `isInteractionCancelled` added to `execute-confirm-btn`'s disabled binding; the cancelled record is kept until window dismissal (overlay/WindowManager cleanup intact); testids on the overlay (root + OK btn). Pins: BOTH orderings + cancel-BEFORE-popup-load + disabled + typed rejection surface — overlay visibility alone insufficient. The dApp-side e2e driver needs a protocol expansion → ledgered TODO, surfaced in the final report as a scope refinement of goal item 4. | e2e:network |
...
- PR-4 is a PRODUCT change to the dapp-interaction cancel path (durable cancelled state,
  first-claim-wins linearization, typed rejection, confirm-disabled binding) — it adds no
  privileges and closes the security-relevant direction: a post-cancel approve can never start
  execution. PR-2 adds one inert state attribute to DropdownRoot. Everything else is
  test/fixture-only; CI gate definitions untouched.
