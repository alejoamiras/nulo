...
### Phase 2 — chain identity + shared formula (extension services) ✓ (gate passed 2026-08-18: lint 0 / typecheck:all 0 / test:all all-green; import-drivers helper deliberately deferred to P4 with its fixtures — lessons/phase-2.md)
...
### Phase 3 — profile entropy model ✓ (gate passed 2026-08-18: lint 0 / typecheck:all 0 / test:all all-green; encrypted-key + plain-key cuts pulled forward from P4 — lessons/phase-3.md; crypto-rider consult below)
...
### Phase 4 — product cuts + copy ✓ (gate passed: audit:vue green 2026-08-18; smoke `test:e2e` green 2026-08-19 — 80 pass + migration-fixture 3/3 armed; 2 real regressions found+fixed — lessons/phase-4.md)
...
### Phase 5 — Export/Import Account ✓ (gate passed: audit:vue green 2026-08-19; smoke `test:e2e` green 2026-08-19 — lessons/phase-5.md)
...
### Phase 6 — reconciliation + network e2e ✓ (gate passed 2026-08-19: prover-ON canary + reimport-matrix green via e2e:agent, 5 passed exit 0 — lessons/phase-6.md)
Canary run (v2 recompute, Full-Backup capture, prover-ON); reimport-matrix run; **imported-account signing leg** (audit M8 — downgraded to unit coverage in `import-export.test.ts` + covered live by the identical-`NuloAccount`-shape canary; `signingKeyHex` affordance left in place); docs; full suite deferred to CI's required `network-e2e-status` gate.
**Gate** — `bun run e2e:agent tests/e2e/network/frozen-account-canary.test.ts tests/e2e/network/profile-reimport-matrix.test.ts` (prover-ON) → 5 passed, exit 0; `bun run audit:vue` exit 0. Layers: e2e-live-network (KDF-relevant subset, real proving) + fast layers. (Owner memory: run the network suite solo on the host; re-run before triaging failures.)
