...
> **Corrections (post-audit, R1):** four claims below were corrected by the dual audit — (1) `NuloAccount`'s constructor is **public** today (nulo-account.ts:46), not private; (2) `createAccountInternal`'s next-index guard is **cross-type** (`accounts.length > 0` on the unfiltered list; `array_max([])` returns 0), so the first account of a new type gets index 1 — the per-type sequence is not "free"; (3) the integrity coordinator's production `DeriveAddress` default **duplicates the v1 seed formula** (coordinator.ts:52-56) and is constructed without an override — it is NOT "reuse as-is" for this plan; (4) "the only formula-coupled test is the canary" is **false** — `tests/e2e/helpers/import-drivers.ts:223-232` hand-codes the formula (feeding import-paths/import-dead-rpc/backup-migration), `tests/e2e/network/profile-reimport-matrix.test.ts` depends on it plus the deleted plain-key UI, and epoch-3 literals sit in `import-drivers.ts`'s `buildSyntheticBackup` and `passkey-backup.test.ts:135`. Also resolved: `getEntropy` DOES reject bad checksums (mnemonic.ts:2150-2157); seeded networks are built **without any probe** (`DEFAULT_SEEDS` → `_buildNetwork`), custom networks always probe. plan.md's Assumptions section carries the corrected facts.
