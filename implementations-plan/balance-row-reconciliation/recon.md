...
|---|---|---|
| Per-profile account read, all chains, one call | `account/service.ts:564-567` `getAccountsRaw(profileId)` — lock-free, no visibility parameter; already used from a boot path at `account-integrity/coordinator.ts:93`. **Missed by this sweep; found by the fable audit** | **reuse-as-is** — replaces N per-chain `getAccounts` calls and removes the `all: true` footgun structurally |
...
| Kill the SW at an arbitrary moment | `stopServiceWorker` — near-identical copies in **8** files (`sw-resilience.test.ts:29-56`, `sw-restart-network.test.ts`, `imported-account-lifecycle.test.ts`, `network/connect-locked-queue-sw-restart.test.ts`, `network/cold-wake-discovery.test.ts`, `network/backup-restore-sw-restart.test.ts`, `network/passkey-execution-canary.test.ts`, `network/frozen-account-canary.test.ts`) — **corrected from 5 by the fable audit; verified**. Uses `worker.close()` (the only Puppeteer call that actually terminates an MV3 SW) + an identity-keyed `targetdestroyed` listener | **reuse-as-is in spirit** — but only available by copy-paste today |
...
- **Do not copy `stopServiceWorker` a 6th time** if an e2e needs it — a new copy would be the NINTH; consolidating is the cheaper drive-by.
