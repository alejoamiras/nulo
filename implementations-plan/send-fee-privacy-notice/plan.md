...
- otherwise → **"Couldn't check your private gas. Pick a fee source to continue."** under a private origin;
  under a public origin the row is absent (D32).
...
| D31 | Forced mount read, every origin | post-implementation codex pass (P1) | Rev 5 forced the mount read only under a private origin. The origin can flip to private inside one mount with no further read (D-"origin flips re-resolve with no fetch"), so a Send card mounted under a public origin could default a later private send to Fee Juice on a TTL-cached zero. Every Send mount (`originPrivacy !== null`) now reads fresh; the `null`-origin cards are unchanged. |
| D32 | Public-origin hold on a healthy store is silent | post-implementation codex pass (P2) | The implementation had reused "…retrying in the background" there; nothing retries on a healthy store, so the line was false. The approved private-gas line does not fit either: under a public origin every self-paid row is disabled, so "Pick a fee source" is not actionable. No new copy was invented — the row is absent and the trigger's existing "select a method" prompt stands. Needs an unread balance on a listed payer, no sponsor and a non-degraded store at once. Flagged to the owner in the PR. |
