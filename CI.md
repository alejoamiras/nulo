...
- `build-extension` — chrome + firefox builds, uploaded as the `extension-chrome` / `extension-firefox` artifacts (7-day retention) with the version stamped `X.Y.Z-pr.<N>` (`version_suffix`). The advisory `preview-comment` job keeps ONE sticky comment on the PR linking to them (edited in place on every push, so the link always matches the current head) — download, unzip, **Load unpacked**. Same-repo PRs only; not in `quality-status`'s `needs`, so it can never red the gate. Logic in `scripts/ci-cd/preview-comment.ts`.
