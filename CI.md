...
| PR touching the tools graph (`apps/tools/**`, the bridge contracts, `packages/bridge-core/**`, its dep libs) or labeled `e2e:tools` | `pr-tools-e2e` (`tools-e2e-status`, advisory today): 4 shards, each on its own sandbox | 15–25 min |
