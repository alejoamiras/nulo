...
- **No shipped file the Firefox linter parses may reach 4.5 MiB.** `addons-linter` refuses to parse a `.js`/`.json`/`.html` file of 5 MiB or more and reports it as an error — and an unparsed file is an unscanned one. Two mechanisms keep the bundle under it, two guards fail the build when they stop being enough: `scripts/vendor-chunks.ts` cuts the proving/simulation/contract packages into one chunk per package, each JSON artifact into a chunk of its own (`@aztec/wallet-sdk` excluded, so the content script's web-accessible chunks stay as they were) — **never cut by size** (`maxSize`): a size cut ignores import cycles, and chunks that import each other build fine and then read each other's bindings as `undefined` in the browser, which is why `scripts/chunk-cycle-guard.ts` fails the build on any static import cycle between chunks; `scripts/strip-artifact-debug-info.ts` empties `file_map` and `debug_symbols` in the artifacts listed in `debugStrippedArtifacts` (`vite.shared.ts`), because a JSON import is one module no bundler can split; `scripts/parse-limit-guard.ts` fails either browser's build, naming the file. When the parse-limit guard fires on an artifact, add it to `debugStrippedArtifacts` (its class id does not depend on debug info — the paired test proves it per artifact); if a stripped artifact, or a single package's code, is still over, ship it compressed and load it at runtime. Never raise the limit. After any change to the split, diff `web_accessible_resources` in both built manifests: every file in it is readable by every page.
