...
| `manifest/` | Per-target manifest configs (Chrome + Firefox). Icons come from `src/assets/icons/{16,32,48,96,128}.png`, generated by `scripts/store-icons.ts`. |
| `store/` | Store submission material: `listing.md` (one source for both dashboards, tested by `scripts/store-listing.test.ts`), `remote-code.md`, the promo tile and framed screenshots (`scripts/store-art.ts`), `captures/` (written by the opt-in `tests/e2e/store-captures.test.ts`). |
...
| `bun scripts/store-icons.ts` | Regenerates the manifest icon set from `src/assets/logo.png`; `--check` fails on drift (also run by `scripts/store-icons.test.ts`). |
| `bun scripts/store-art.ts` | Renders `store/promo-440x280.png` and `store/screenshot-{1,2,3}-1280x800.png` from `store/templates/` and `store/captures/`. |
