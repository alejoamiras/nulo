...
- **No shipped file the Firefox linter parses may reach 4.5 MiB.** `addons-linter` refuses to parse a `.js`/`.json`/`.html` file of 5 MiB or more and reports it as an error — and an unparsed file is an unscanned one. Two mechanisms keep the bundle under it, one fails the build when they stop being enough: `vite.config.ts` splits `node_modules` into entry-aware vendor chunks (`@aztec/wallet-sdk` excluded, so the content script's web-accessible chunks stay as they were); `scripts/strip-artifact-debug-info.ts` empties `file_map` and `debug_symbols` in the artifacts listed in `debugStrippedArtifacts` (`vite.shared.ts`), because a JSON import is one module no bundler can split; `scripts/parse-limit-guard.ts` fails either browser's build, naming the file. When the guard fires on an artifact, add it to `debugStrippedArtifacts` (its class id does not depend on debug info — the paired test proves it per artifact); if a stripped artifact is still over, ship it compressed and load it at runtime. Never raise the limit. After any change to the split, diff `web_accessible_resources` in both built manifests: every file in it is readable by every page.
