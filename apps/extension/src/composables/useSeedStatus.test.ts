...
function harness() {
	...
	const client = {
		getSeedStatus: vi.fn(async (): Promise<SeedStatusEntry[]> => [entry("pending")]),
...
describe("useSeedStatus", () => {
	...
	test("a status event for this scope refetches; one for another scope is ignored", async () => {
		...
		client.getSeedStatus.mockResolvedValueOnce([])
	...
	test("only the latest request lands: an older answer resolving late is dropped", async () => {
		const { seed, client } = harness()
		const slow = deferred<SeedStatusEntry[]>()
		client.getSeedStatus.mockReturnValueOnce(slow.promise).mockResolvedValueOnce([entry("seeding")])
		const first = seed.refresh()
		await seed.refresh()
		slow.resolve([entry("failed")])
		await first
		expect(seed.entries.value).toEqual([entry("seeding")])
	})
	...
	test("a scope change clears the old rows at once, goes back to loading, and kicks the new scope", async () => {
		...
		// The worker answered for the scope it saw a moment earlier: rows of another chain are not ours.
		pending.resolve([entry("pending", 1), entry("pending", 2)])
		await refetch
	...
	test("retry calls the service for that default, then shows whatever is true — even when the call rejects", async () => {
		const { seed, client } = harness()
		await seed.refresh()
		client.getSeedStatus.mockResolvedValueOnce([entry("seeding")])
