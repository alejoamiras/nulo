 ...
 *      migration bumps the DURABLE attempt counter whose cadence is next-boot
 *      by design — an in-lifetime retry loop driven by the price alarm would
 *      burn the whole cross-boot budget in minutes);
 *    - a Barretenberg init failure (`BarretenbergSync.initSingleton` memoizes
 *      its REJECTED promise upstream with no reset — verified in the vendored
 *      source — so a retry can only re-observe the same error);
 *    - the registration zone (`ServiceCollection.add` throws on duplicates, and
 *      the tabs/pxe-provider registrations are not re-entrant).
 *  A vetoed failure keeps the rejected memo: callers observe the rejection, and
 *  a fresh SW lifetime — fresh module state — is the retry. What remains
 *  genuinely retryable: transient storage writes — the schema-status
 *  sets/removes and config.load's own apply() write, all of which can throw
 *  transiently and re-run safely. */
...
export function createWalletRuntime(deps: WalletRuntimeDeps): WalletRuntime {
	...
	return {
		start,
		stop: () => stopRuntime(state, clock, logger),
		get services() {
			return services
		},
	}
...
export function stopRuntime(state: RuntimeState, clock: WalletRuntimeDeps["clock"], logger: WalletRuntimeDeps["logger"]): void {
	if (state.heartbeatHandle !== undefined) {
		clock.clearInterval(state.heartbeatHandle)
...
/** The boot sequence — the single-flight memo's body. Phase order is the
 *  contract: uninstall URL → migration gate → engine → outcome persistence →
 *  config + Barretenberg → registration → `services.start()` (with the journal
 *  boot cutoff captured IMMEDIATELY before it) → post-start work → wallet-sdk
 *  handler → first liveness write → heartbeat (deliberately last). */
async function bootRuntime(deps: WalletRuntimeDeps, services: ServiceCollection, state: RuntimeState): Promise<void> {
	...
	try {
		gateDecision = await evaluateMigrationGate(deps)
	} catch (error) {
	...
	const migration = await runMigrationEngine(browserApi)
	...
	if (migration.kind === "needs-recovery" || (migration.kind === "failed" && migration.breaking)) {
		...
		// The veto lands BEFORE the status write is awaited: a rejected write
		// must not leave the retry permitted.
		state.retrySafe = false
		await persistBlockedOutcome(migration, gateDecision, deps)
	} else if (migration.kind === "failed") {
		await persistDegradedOutcome(migration, deps)
	} else {
	...
	await loadConfigAndBarretenberg(config, logger, () => {
		state.retrySafe = false
	})

	// Registration zone: `ServiceCollection.add` throws on duplicates and the
	// tabs/pxe-provider registrations are not re-entrant — no retry past here.
	state.retrySafe = false
	const { deletionCoordinator } = registerServices(services, deps)
 ...
 */
async function evaluateMigrationGate(deps: WalletRuntimeDeps): Promise<MigrationGateDecision> {
	const { browserApi, clock, manifestVersion } = deps
 ...
 *  config) and before any service reads storage. The engine contractually
 *  never throws (a throw becomes a retryable needs-recovery result) — this
 *  catch is belt-and-braces so even an engine BUG still lands on the
 *  blocked-status recovery UX instead of a bare boot crash. It deliberately
 *  clears NOTHING: an armed backup may be load-bearing, and the engine's
 *  next-boot resume owns journal cleanup for every stranded shape. */
function runMigrationEngine(browserApi: WalletRuntimeDeps["browserApi"]): Promise<MigrationResult> {
	return new Migrator({
...
/** A blocking migration outcome: needs-recovery, or a failed BREAKING migration. */
type BlockedMigration = Extract<MigrationResult, { kind: "needs-recovery" | "failed" }>

/** Persist the blocked status so the UI shells render the recovery screen
 *  (MigrationBarrier) instead of a dead popup, then fail closed: never start
 *  services on un-migrated / incompatible data. Always throws. */
async function persistBlockedOutcome(
	migration: BlockedMigration,
	gateDecision: Extract<MigrationGateDecision, { action: "run" }>,
	deps: WalletRuntimeDeps,
): Promise<never> {
	const { browserApi, clock, logger, manifestVersion } = deps
...
/** An additive migration failed: boot degraded. A stale blocked status from
 *  an EARLIER boot must not outrank the degraded banner over a wallet that
 *  just booted. The ATTEMPTS clear matters too: without a blocked key the
 *  gate runs the engine on every ambient wake, and a lingering counter would
 *  let those wakes silently exhaust the budget — a later restore failure would
 *  then arrive instantly terminal with zero gestures. */
async function persistDegradedOutcome(migration: Extract<MigrationResult, { kind: "failed" }>, deps: WalletRuntimeDeps): Promise<void> {
 ...
 *  Plain Promise.all (not allSettled): a BB failure vetoes the memo so no
 *  retry can overlap a still-pending config leg, and a config rejection (its
 *  apply() storage write can throw) settles that leg itself before the retry
 *  re-runs it — while allSettled would leave the boot promise silently pending
 *  forever when a leg hangs after the other failed. `onBarretenbergFailure`
 *  runs synchronously inside the BB catch, before the rejection propagates. */
function loadConfigAndBarretenberg(
	config: WalletRuntimeDeps["config"],
	logger: WalletRuntimeDeps["logger"],
	onBarretenbergFailure: () => void,
): Promise<unknown> {
	return Promise.all([
		...
		BarretenbergSync.initSingleton({ wasmPath: process.env.BB_WASM_PATH })
			...
			.catch((err) => {
				...
				onBarretenbergFailure()
				throw err
			}),
	])
}

/** Service graph. Services migrated to ports accept `browserApi`; remaining
 *  services still reach into `chrome.*` directly until their migration lands.
 *  Registration order here is a visual convention only — actual startup
 *  ordering is determined by `ServiceCollection.start()`'s topological phases.
 *  Fully synchronous: the PXE provider CALLBACKS registered here run later,
 *  off the boot chain. */
function registerServices(services: ServiceCollection, deps: WalletRuntimeDeps): { deletionCoordinator: ProfileDeletionCoordinator } {
	const { browserApi, clock, config, logger } = deps
...
/** Everything that fires right after `services.start()` resolves, in this
 ...
 *  wedge startup. The setInterval delays its FIRST fire by
 *  HEARTBEAT_INTERVAL_MS, so without this immediate write anything waiting on
 *  the liveness signal (popup pages, e2e fixtures) sees a 10s gap on cold
 *  boot. Placed AFTER the SDK handler so liveness means "runtime is fully
 *  wired and ready to handle messages." */
function writeInitialLiveness(
	browserApi: WalletRuntimeDeps["browserApi"],
	clock: WalletRuntimeDeps["clock"],
	logger: WalletRuntimeDeps["logger"],
): void {
	browserApi.storage.session
		.set({ "nulo:liveness": clock.now() })
