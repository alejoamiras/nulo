...
describe("send-submit", () => {
	...
	test("resolved: the success toast, the row kept, then settled", async () => {
		...
		settle().resolve()
		await flush()
		expect(deps.openToast).toHaveBeenCalledWith({ label: "Transaction submitted", icon: "check-circle" })
	...
	test("rejected: this row removed, a red toast, logged as an error, then settled", async () => {
		...
		expect(deps.openToast).toHaveBeenCalledWith({ label: TRANSFER_FAILED_COPY, icon: "warning", color: "red" }, 4000)
	...
	test("refused by the terms wall: the terms copy, logged at debug only", async () => {
		...
		await flush()
		expect(deps.openToast).toHaveBeenCalledWith({ label: TRANSFER_TERMS_COPY, icon: "warning", color: "red" }, 4000)
