...
const bal = (id: string, account: string): TokenBalanceEvent => ({ id, account })
...
describe("applyBalanceAdd (B-25)", () => {
	...
	test("appends a balance for the active account", () => {
		const list = [bal("t1", "0xacc")]
		applyBalanceAdd(list, "0xacc", bal("t2", "0xacc"))
		expect(list.map((b) => b.id)).toEqual(["t1", "t2"])
	})

	test("ignores a balance for a different account", () => {
		const list = [bal("t1", "0xacc")]
		applyBalanceAdd(list, "0xacc", bal("t2", "0xother"))
		expect(list.map((b) => b.id)).toEqual(["t1"])
	})
})

describe("applyBalanceUpdate", () => {
	test("replaces an existing balance in place, matched by id", () => {
		const list = [bal("t1", "0xacc"), bal("t2", "0xacc")]
		const replacement = { ...bal("t2", "0xacc"), extra: 1 } as TokenBalanceEvent
	...
	test("ignores an unknown id (adds arrive via applyBalanceAdd)", () => {
		const list = [bal("t1", "0xacc")]
		applyBalanceUpdate(list, bal("t9", "0xacc"))
		expect(list.map((b) => b.id)).toEqual(["t1"])
	})
})
