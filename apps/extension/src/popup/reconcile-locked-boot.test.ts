...
describe("reconcileLockedBoot", () => {
	...
	test("an unlock event lands while the lookup is in flight: the boot run applies nothing", async () => {
		const { state, lookup, run } = shell()
		...
		expect(await run).toEqual({ kind: "event-superseded" })
		expect(state.calls).toEqual([])
	})

	test("the lookup resolves first, then an event lands before the run reaches its action: still nothing", async () => {
		const { state, lookup, run } = shell()
		lookup.resolve(locked)
	...
	test("a lock event from another window deletes the listed profile: the stale candidate is never selected", async () => {
		// An import-page shell with no selected profile lists [P]; another window deletes P and its
		// lock event lands here (routing this shell to register); the boot's session read then
		// returns locked with P as the candidate. Selecting P would resurrect a deleted profile.
		const { state, lookup, run } = shell({ decide: () => "select-and-auth" })
		lookup.resolve(locked)
		state.eventSeq += 1
		expect(await run).toEqual({ kind: "event-superseded" })
		expect(state.calls).toEqual([])
	...
	test("the decision is read at action time, after the fence: a store mutated by the event path never reaches it", async () => {
		let decided = 0
		const { state, lookup, run } = shell({
			decide: () => {
				decided += 1
	...
	test("without an event, each action runs; passkey-hold touches nothing", async () => {
