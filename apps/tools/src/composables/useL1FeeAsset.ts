...
export function useL1FeeAsset() {
	...
	watch(
		() => l1.address.value,
		(addr) => {
			void refreshBalance(l1)
			if (addr) ensurePolling(l1)
		},
		{ immediate: true },
	)
	...
	return {
		...
		minting,
		mintError,
		refresh: (): Promise<void> => refreshBalance(l1),
		allowance: (): Promise<bigint> => readAllowance(l1),
		approve: (amount: bigint): Promise<void> => approveFeeAsset(l1, amount),
		mint: (): Promise<void> => mintFeeAsset(l1),
		verifyPortalAsset: (): Promise<void> => verifyPortalAsset(l1),
	}
}

type L1Wallet = ReturnType<typeof useL1Wallet>

async function refreshBalance(l1: L1Wallet): Promise<void> {
...
async function readAllowance(l1: L1Wallet): Promise<bigint> {
	const owner = l1.address.value
...
async function verifyPortalAsset(l1: L1Wallet): Promise<void> {
...
async function approveFeeAsset(l1: L1Wallet, amount: bigint): Promise<void> {
...
async function verifyHandlerAsset(l1: L1Wallet): Promise<void> {
...
async function mintFeeAsset(l1: L1Wallet): Promise<void> {
	const wallet = l1.ensureWalletClient()
	const owner = l1.address.value
	if (!wallet || !owner) {
...
function ensurePolling(l1: L1Wallet): void {
	if (timer !== null) return
	timer = setInterval(() => void refreshBalance(l1), POLL_INTERVAL_MS)
}
