 ...
 *   2. The chain constants (`chain-constants.ts`).
 *
 ...
 * single-sourced from `chain-constants.ts` so it cannot drift.
 *
...
export function readChainInfo(url: URL = new URL(window.location.href)): ChainInfo {
	...
	return {
		chainId: parseField(String(TESTNET_L1_CHAIN_ID)),
		version: parseField(String(TESTNET_ROLLUP_VERSION)),
	}
}
