...
- **The baseline only shrinks — NEVER add a complexity suppression.** ~230 pre-existing offenders carry function-scoped `// biome-ignore lint/complexity/...: baseline (...)` directives, pinned per-rule × per-file by [`scripts/complexity-baseline/manifest.json`](./scripts/complexity-baseline/manifest.json). Enforced twice: `scripts/complexity-baseline/check.ts` runs inside `bun run lint` and the pre-commit hook (sub-second git-grep scan), and `scripts/ci-cd/complexity-baseline.test.ts` mirrors it in `test:ci-gating` — so a new suppression reds the local loop AND CI. When you touch a baselined function, refactor it under budget and rerun `bun run baseline:complexity` in the same PR (the shrink is otherwise a check failure — the manifest must match exactly).
...
- **Duplication is watched, not gated.** `bun run audit:dup` (jscpd, exact-pinned in `scripts/dup-trend/report.ts`) prints the duplication trend; nightly's advisory `dup-trend` job pipes it into the step summary. Duplication is the most consistent LLM-slop signal, but clone identity is too unstable to ratchet (boundaries shift whenever either side is edited) — the escalation, a diff-scoped new-clone check, gets built only if the trend worsens after the complexity burn-down.
