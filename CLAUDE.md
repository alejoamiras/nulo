...
This repo ships **one product, the Nulo wallet** (`apps/extension`), plus what serves it: the generic test dApp [`apps/playground`](./apps/playground/README.md), the landing (`apps/landing`) and the `packages/*` layers. The any-ERC-20 bridge, its tools app, their CI and their plans moved to [`alejoamiras/unleashed`](https://github.com/alejoamiras/unleashed); until it is populated, read them at [`nulo@<FREEZE_SHA>`](https://github.com/alejoamiras/nulo/tree/<FREEZE_SHA>).
