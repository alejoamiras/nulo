...
- **`extension-smoke-e2e-firefox-status`** / **`extension-network-e2e-firefox-status`** (`pr-extension-{smoke,network}-e2e-firefox.yml`): the same two suites on Firefox (geckodriver + Puppeteer over BiDi), behind the same filters and labels as their Chrome twins plus their own files; draft PRs skip them. Four files are Chrome-only and skip whole-file there: one kills the background under an open extension page (Firefox will not end its event page while one is open), one redirects over CDP Fetch, and the two execution canaries are pinned to Chrome (`CHROME_ONLY` in `fixtures/browser/index.ts` carries each reason). Every other background-kill spec runs on both browsers through the driver's `stopBackground`. **Advisory today**: in neither branch's required set, and `behavior-gating.test.ts` pins that no aggregator or publish step waits on a Firefox job and that each lane runs exactly its Chrome twin's files. `nightly.yml` and `release.yml` carry advisory Firefox jobs too (`smoke-firefox-against-artifact` smokes the shipped Firefox zip). Promotion is staged — see [§ Staged-rollout switches](#staged-rollout-switches).
