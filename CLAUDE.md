...
- **Every dependency the extension bundles must pass the third-party notices policy.** The build emits `THIRD-PARTY-NOTICES.txt` from what it actually rendered (main + worker builds) and REFUSES a licence outside the allowlist, a package with no licence metadata or no licence file, a stale or unused override, and an unclaimed compiled asset — see [`packages/third-party-notices/README.md`](./packages/third-party-notices/README.md). Loosening `ALLOWED`, or adding an `OVERRIDES` / `VENDORED` record without a tagged upstream source, is weakening a quality gate. The generator never writes a copyright line; neither do you.
