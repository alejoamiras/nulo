...
- **The baseline only shrinks — NEVER add a complexity suppression.** The steady-state residue is **49 directives** (230 at adoption → 142 after the 2026-09 round-1 arcs → 49 after round 2's seven plans; records in `implementations-plan/{profile-service-dedup,cognitive-shallow-tail,deposit-decomposition,backup-import-decomposition,journal-engine-decomposition,vue-shell-composites}/` and `implementations-plan/{backup-integrity,pxe-network-boundary,execution-pipeline,balance-durable-jobs,sw-wallet-protocol,faucet-cluster,popup-shell-state}/`), each a function-scoped `// biome-ignore lint/complexity/...: baseline (...)` directive, pinned per-rule × per-file by [`scripts/complexity-baseline/manifest.json`](./scripts/complexity-baseline/manifest.json). Every one of the 49 is on the owner-signed accept list in [`implementations-plan/complexity-residue-round-2/scope.md`](./implementations-plan/complexity-residue-round-2/scope.md): 13 production functions whose branches ARE their specification (the redaction walker, the hostile-JSON migration algebra, the integer formatter, the Kahn layering, the recursive codecs, two precedence policies, two length-only aggregates), 11 run-once bridge-core scripts, and 25 test/e2e harness directives whose scores mirror scenario matrices. "Refactor when touched" is each directive's contract; moving a row off that list is a blueprint, and a NEW accept above score 35 needs owner sign-off. Enforced twice: `scripts/complexity-baseline/check.ts` runs inside `bun run lint` and the pre-commit hook (sub-second git-grep scan), and `scripts/ci-cd/complexity-baseline.test.ts` mirrors it in `test:ci-gating` — so a new suppression reds the local loop AND CI. When you touch a baselined function, refactor it under budget and rerun `bun run baseline:complexity` in the same PR (the shrink is otherwise a check failure — the manifest must match exactly). **The generator INSERTS directives for functions a refactor pushes over budget** — a net-shrinking regen can mask a new offender, so read the regen diff, never just the count.
- **Only exact single-rule directives are legal.** The scanner classifies every Biome suppression form that could reach a budget rule and forbids the broad ones outright: bare `biome-ignore lint:`, group `biome-ignore lint/complexity:`, and any `-all`/`-start` (file-wide/range) variant covering these rules — each verified to suppress on Biome 2.5.9 while looking innocent. (The generated `src/types/*.d.ts` headers keep their sanctioned bare suppressions — they're outside Biome's lint scope and the scan alike.)
- **A `@biomejs/biome` version bump requires baseline regeneration** in the same PR (`bun run baseline:complexity -- --adopt`) — scores drift between releases, and the manifest pins the version, so the checks red until you do. Regeneration re-counts and (under `--adopt` only) grandfathers functions the new version newly flags; directives made stale by a bump surface as Biome `suppressions/unused` warnings — delete them and regenerate. Without `--adopt` the generator refuses to grow the baseline at all.
