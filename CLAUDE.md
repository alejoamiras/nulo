...
- **The baseline only shrinks — NEVER add a complexity suppression.** The steady-state residue (~140 directives, down from 230 at adoption via the 2026-09 burn-down arcs — see `implementations-plan/{profile-service-dedup,cognitive-shallow-tail,deposit-decomposition,backup-import-decomposition,journal-engine-decomposition,vue-shell-composites}/`) carries function-scoped `// biome-ignore lint/complexity/...: baseline (...)` directives, pinned per-rule × per-file by [`scripts/complexity-baseline/manifest.json`](./scripts/complexity-baseline/manifest.json). What remains is deliberate: bridge-core operational scripts, test/e2e harnesses whose scores mirror scenario matrices, and large prod services awaiting their own decomposition ("refactor when touched" is each directive's contract). Enforced twice: `scripts/complexity-baseline/check.ts` runs inside `bun run lint` and the pre-commit hook (sub-second git-grep scan), and `scripts/ci-cd/complexity-baseline.test.ts` mirrors it in `test:ci-gating` — so a new suppression reds the local loop AND CI. When you touch a baselined function, refactor it under budget and rerun `bun run baseline:complexity` in the same PR (the shrink is otherwise a check failure — the manifest must match exactly). **The generator INSERTS directives for functions a refactor pushes over budget** — a net-shrinking regen can mask a new offender, so read the regen diff, never just the count.
