...
- **The any-ERC-20 bridge and its tools app moved to [`alejoamiras/unleashed`](https://github.com/alejoamiras/unleashed).** Until it is populated, read them at the freeze commit: [`packages/bridge-core`](https://github.com/alejoamiras/nulo/tree/6611f8611100931fe266f6fd1dfff27e331e2897/packages/bridge-core) (the generation model, the conductor + verifier scripts, the salt/leaf/portal invariants), [`contracts/bridge`](https://github.com/alejoamiras/nulo/tree/6611f8611100931fe266f6fd1dfff27e331e2897/contracts/bridge) (the factory / clone / router threat model, the hub, the split Noir/JS toolchain), [`apps/tools`](https://github.com/alejoamiras/nulo/tree/6611f8611100931fe266f6fd1dfff27e331e2897/apps/tools) (the tools app + Send wizard). **§ The wallet repo** below says what still crosses the repo line.
...
This repo ships **one product, the Nulo wallet** (`apps/extension`), plus what serves it: the generic test dApp [`apps/playground`](./apps/playground/README.md), the landing (`apps/landing`) and the `packages/*` layers. The any-ERC-20 bridge, its tools app, their CI and their plans moved to [`alejoamiras/unleashed`](https://github.com/alejoamiras/unleashed); until it is populated, read them at [`nulo@6611f8611100931fe266f6fd1dfff27e331e2897`](https://github.com/alejoamiras/nulo/tree/6611f8611100931fe266f6fd1dfff27e331e2897).
...
- **The playground stays generic.** It reaches the wallet only through `@aztec/wallet-sdk` (plus the fail-open custom RPCs) and never imports from `apps/extension/src`; the extension never special-cases it. Otherwise a green e2e proves the pair, not the wallet.
...
- **Nothing here imports, fetches or reads unleashed; what crosses the line is data.** Chain identity: each repo keeps its own constants (here `apps/extension/src/utils/chain-ids.ts`). The PrivateFPC canonical address: pinned wallet-side by `apps/extension/src/wallet/services/fpc/protocol-fpcs.test.ts`; its on-chain deploy moved with the bridge. The bridged-USDC seeds in `default-tokens.ts` / `price-map.ts` are the **retired** single-token bridge's L2 tokens and mirror nothing in unleashed: re-pointing them is an owner UI decision, and a future mirror would land in the `aztec-update` skill's reset step. User-facing links: the fee card's `FEE_JUICE_BRIDGE_URL` default (`https://tools.nulo.sh`) and the landing's Tools links (`testnet.tools.nulo.sh`) point at the bridge's hosts, which serve the paused tools deployment until the cut-over redirects them to unleashed; changing the links is an owner UI decision too.
- **Three packages are also staged for npm** by [`scripts/publish/`](./scripts/publish/README.md) as `@alejoamiras/nulo-wallet-crypto` (key derivation + `EncryptionKey` only), `-resolve-asset` and `-wallet-sdk-schema-patch`: the surface unleashed consumes. The workspaces stay `private` under their `@nulo/*` names; widening a published surface (`packages/wallet-crypto/src/public.ts`, `scripts/publish/packages.ts`) is an npm API change. Publishing is `publish-packages.yml`, owner-approved, provenance-attested and tokenless.
...
| Anything bridge-side — a generation (factory + router + hub), its contracts, the tools app | not this repo: [`alejoamiras/unleashed`](https://github.com/alejoamiras/unleashed). Until it carries them, read [the `aztec-update` skill at the freeze commit](https://github.com/alejoamiras/nulo/blob/6611f8611100931fe266f6fd1dfff27e331e2897/.claude/skills/aztec-update/SKILL.md): Branch B's generation runbook, and Phase 1's Noir surface and bridge drift detectors |
...
- **Bun** is the package manager. No yarn/npm/pnpm. Pinned to `1.4.2` via `package.json#packageManager` + `setup-bun` action. **Local minimum is 1.4** — the parallel scripts (`audit:vue`, `dev:full`) use `bun run --parallel`. `bun.lock` is `lockfileVersion: 2` (regenerated under the isolated linker), which **Bun ≤1.3 cannot read** — every host that installs must run ≥1.4: local, CI (`setup-bun`), the landing's Cloudflare Pages project `nulo` (`BUN_VERSION` env var, Production AND Preview — the Pages image defaults to Bun 1.2.15 and honors no version file), and the landing Worker `nulo-landing`'s Workers Builds (`BUN_VERSION` build variable) once it is connected — both build until Pages is deleted.
...
- Both branches require **signed commits** (SSH or GPG — the merge's squash commit is GitHub web-flow-signed, CLI or UI). **Both `dev` and `main` require all three status gates — `quality-status`, `extension-network-e2e-status`, and `extension-smoke-e2e-status`** (the *produced* check-run names — a GitHub Actions normal job's check-run is named its bare job `name:`, with no `Workflow /` prefix), each app_id-pinned to GitHub Actions in `required_status_checks.checks`. Both branches are cut over to the `extension-*` names: `dev` on 2026-09-09, `main` on 2026-09-23. Smoke is **required** on both, not advisory. The two Firefox aggregators (`extension-smoke-e2e-firefox-status`, `extension-network-e2e-firefox-status`) are produced but **advisory**, and promoting either into a branch's required set is the owner's call, through the same runbook. **Check names say what they gate** (`extension-*`; `quality-status` is repo-wide) — a renamed aggregator job blocks every merge on that branch until the protection is repointed, so renames ship with the runbook: `scripts/ci-cd/required-checks.sh print --branch dev --json > <file>` (review it), then, right before merging the rename, `scripts/ci-cd/required-checks.sh --apply --branch dev --expect <file>`; `main` gets the same two steps at its next promote. `scripts/ci-cd/behavior-gating.test.ts` pins each PR workflow's aggregator name to the list the runbook renames onto.
...
- **`@aztec/*` outside the policy** — exact-pinned, bumped manually. **Any Aztec version bump follows the `aztec-update` skill** ([`.claude/skills/aztec-update/SKILL.md`](./.claude/skills/aztec-update/SKILL.md)): it classifies the bump first (version-only vs a NETWORK RESET, which moves the wallet's chain identity), then walks the full pin surface (`@alejoamiras/presto` + the held `@aztec-foundation/aztec-standards` / `@alejoamiras/private-fee-juice` pair + patches + min-age excludes), the drift detectors and the execution canaries, and — on a reset — the wallet side only: the chainId cascade and the client-side reset (storage baseline, per-chain purge). The bridge's generation redeploy belongs to `alejoamiras/unleashed`. The running list of types coupled to the `@aztec` shape is logged in [`UPDATE.md`](./UPDATE.md).
...
- **Bun-version Renovate PRs need manual sync**: Renovate bumps `package.json#packageManager` but NOT `.github/actions/setup-bun/action.yml` (the version input + its two cache-key occurrences), nor the four `bun-version:` literals on the store jobs (`release.yml`'s `publish-chrome-store` / `publish-firefox-amo`, `store-check.yml`'s two jobs), nor the out-of-repo sites: the `BUN_VERSION` env var on the landing's Cloudflare Pages project (`nulo` × Production + Preview) and the `BUN_VERSION` build variable on the `nulo-landing` Worker's Workers Builds — both dashboard-only. Existing CI (`_lint-and-typecheck.yml`) won't catch the drift — review the PR's diff for every one of them and keep all the sites on one line. A Bun bump also requires the machine-wide local bun ≥ the pinned line before merge (the `--parallel` scripts need it, and the v2 lockfile needs ≥1.4).
...
| Redeploy the landing | `refresh-landing` | ✅ automatic |
| Confirm the landing serves THIS release | `verify-live` | ✅ automatic (advisory) |
...
4. **Wait for the publish chain** (~15–25 min): gates → build → smoke → `attach-assets` → landing deploy → `verify-live`.
...
- `CLOUDFLARE_PAGES_DEPLOY_HOOK` repo secret set (used by the workflow's `refresh-landing` job) while Pages serves `nulo.sh`. The `nulo-landing` Worker that replaces it has no deploy hook: Workers Builds' Git connection (set up in the Cloudflare dashboard) builds `main` for production and other branches as previews, so after the cut-over the landing picks up a release on the next push to `main`. Deleting Pages retires the secret, `refresh-landing`, `refresh-landing.yml` and `verify-live`'s landing check together.
...
7. **Cloudflare landing redeploy — automatic on ANY stable publish path** (`push:main` AND `workflow_dispatch`). The `refresh-landing` job carries an `always() && !cancelled()` guard (fixed 2026-07-03, `implementations-plan/release-pipeline-hardening/`). It used to lack `always()` and so silently skipped whenever a `workflow_dispatch` ancestor (`network-e2e` / `release-please` / `auto-unstick`) skipped — which stranded `nulo.sh` on the prior version after every manual-unstick publish. If a deploy is stale anyway (a CDN miss, or you want to force one), re-fire it with the break-glass workflow — **one command, no CF token, no account id**: `gh workflow run refresh-landing.yml`.
...
| `verify-live` **red** but the release shipped | CDN lag, or a real deploy miss | It's advisory — open `nulo.sh`: its HTML must link `releases/tag/vX.Y.Z` (what `scripts/release/verify-live.ts` asserts). If not, re-fire `gh workflow run refresh-landing.yml`. |
