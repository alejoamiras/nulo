...
| Anything bridge-side — a generation (factory + router + hub), its contracts, the tools app | not this repo: [`alejoamiras/unleashed`](https://github.com/alejoamiras/unleashed). Until it carries the generation runbook, read [`aztec-update` § Branch B at the freeze commit](https://github.com/alejoamiras/nulo/blob/<FREEZE_SHA>/.claude/skills/aztec-update/SKILL.md) |
...
- **Bun-version Renovate PRs need manual sync**: Renovate bumps `package.json#packageManager` but NOT `.github/actions/setup-bun/action.yml` (the version input + its two cache-key occurrences — the only other in-repo pin site now that `pr-quick.yml`'s commitlint job reuses the composite), nor the out-of-repo third site: the `BUN_VERSION` env var on the landing's Cloudflare Pages project (`nulo` × Production + Preview — dashboard-only). Existing CI (`_lint-and-typecheck.yml`) won't catch the drift — review the PR's diff for both files and keep the three sites on one line. A Bun bump also requires the machine-wide local bun ≥ the pinned line before merge (the `--parallel` scripts need it, and the v2 lockfile needs ≥1.4).
