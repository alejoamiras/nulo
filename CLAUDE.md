...
5. **Merge-commit the back-sync PR (NOT squash)** — `sync-main-to-dev` opens `chore: sync main → dev`. **Merge it with a merge commit** so `main`'s release commit stays in `dev`'s ancestry — a squash drops it and re-breaks the next rc cut (this was the [prerelease-fix](implementations-plan/release-prerelease-fix/plan.md)). `dev`'s ruleset now allows `merge` alongside `squash`. Because the merge carries the bot's **unsigned** manifest-rebaseline commit onto `dev`, classic `required_signatures` blocks a plain merge → merge with `gh pr merge <n> --merge --admin` (bypasses the signature check for that one bot commit; the clean fix — an App-API-verified manifest commit — is a tracked follow-up). Labeled `needs-manual-resolution`? Resolve the conflict on the sync branch first, then merge-commit.
...
| `sync-main-to-dev` PR labeled `needs-manual-resolution` | `dev` diverged from `main` since the release | Resolve the conflict on the sync branch (usually `CHANGELOG.md` / `bun.lock`), then **merge-commit** it (`--merge --admin`, NOT squash — preserves release-commit ancestry on `dev`). |
