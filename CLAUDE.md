...
- **Every `@aztec/*` bump PR runs the frozen-account execution canary prover-ON** (`bun run e2e:agent tests/e2e/network/frozen-account-canary.test.ts` — see the `aztec-update` skill). The address KAT cannot see execution breakage (frozen bytecode vs newer simulator/prover/node); the canary is the only gate that does. **A red canary BLOCKS the bump: default response is HOLD the `@aztec` line**; shipping a new extension major is the deliberate alternative.
...
- **`extension-smoke-e2e-firefox-status`** / **`extension-network-e2e-firefox-status`** (`pr-extension-{smoke,network}-e2e-firefox.yml`): the same two suites on Firefox (geckodriver + Puppeteer over BiDi), behind the same filters and labels as their Chrome twins plus their own files; draft PRs skip them. Ten files are Chrome-only by capability (they kill the MV3 service worker or arm CDP Fetch) and skip whole-file there, `frozen-account-canary` among them — both canaries stay Chrome-only. **Advisory today**: in neither branch's required set, and `behavior-gating.test.ts` pins that no aggregator or publish step waits on a Firefox job and that each lane runs exactly its Chrome twin's files. `nightly.yml` and `release.yml` carry advisory Firefox jobs too (`smoke-firefox-against-artifact` smokes the shipped Firefox zip). Promotion is staged — see [§ Staged-rollout switches](#staged-rollout-switches).
