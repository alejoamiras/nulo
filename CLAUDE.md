...
- **`nightly.yml`** (the only scheduled workflow): 03:00 UTC daily from dev — full gate battery (lint/typecheck + units + complete network suite + builds + smoke-against-artifact) then a prerelease GitHub Release tagged `v<version>-nightly.<YYDDD>` (YY+day-of-year because Chrome/Firefox cap manifest version components at 65535). Quiet-day skip when dev HEAD already has tonight's tag; manual dispatch has `force` + `dry_run`. Flake policy is retry-lenient (config-default retries), unlike the PR gates' honest retry-0. See [CI.md § nightly.yml](./CI.md#nightlyyml).
