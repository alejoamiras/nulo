...
- **`minimumReleaseAge = 604800`** (7 days) in `bunfig.toml` — blocks fresh npm publishes. CVE bypass: edit `bunfig.toml` `minimumReleaseAgeExcludes`, install, then delete the exclude again **in the same PR** (locked versions are never re-gated; prove it with `bun install --frozen-lockfile --force`).
