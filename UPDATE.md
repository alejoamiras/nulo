...
- Fee-juice claim phase semantics — `FeeJuice.claim_and_end_setup` is ONLY valid as the fee payload (setup phase, where `FeeJuicePaymentMethodWithClaim` places it); an app-phase claim under a sponsored fee MUST use plain `claim` (`apps/faucet/src/composables/fuelClaim.ts`, `useDeposit.ts`). Live-caught by the direct-FJ canary on 5.0.0.
