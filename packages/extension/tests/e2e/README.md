...
**Quarantined tests**: `tx-sendTx-default`, `multi-account-from`, `tx-sendTx-multicall` (both #32 + #33) are skipped on CI via `NULO_E2E_SKIP_DEFERRED_SLOW=1`. accelerator-server 1.0.1 only covers `createChonkProof`; the init/inner/reset/tail kernel proofs still run via bb.js WASM in-process and exceed puppeteer's `protocolTimeout=300s` ceiling on slow-runner-pool members. Locally they still run (env unset). Tracked in `implementations-plan/network-e2e-unquarantine/`.
