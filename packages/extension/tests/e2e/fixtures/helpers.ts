...
// ── Diagnostic probes (TEMPORARY — network-followups slow-test investigation) ─────
 ...
 *
 * Vitest's default reporter SUPPRESSES console.log output from passing
 * tests; to get a deterministic trace regardless of pass/fail, we append
 * directly to a file at `$TMPDIR/nulo-probes-<runid>.jsonl`. Caller can
 * `cat` it after the run. runId is `process.env.NULO_PROBE_RUN_ID` if
 * set, else current PID. Stripped in Phase C.3 of the network-followups
 * plan.
 */
