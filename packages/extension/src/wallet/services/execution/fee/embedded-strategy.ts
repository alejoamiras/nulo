	...
	public async buildAndEstimate(ctx: FeeStrategyContext): Promise<FeeEstimate> {
		...
		try {
			const { txRequest, node, pxe, account, network, nonce, txCalls, pendingPublicAuthwits } =
				await this.deps.txBuilder.buildStandard(ctx.op, embeddedMethod, task)
			...
			task.complete()
			return { txRequest, node, pxe, account, network, nonce, txCalls, pendingPublicAuthwits, feePaymentMethod: embeddedMethod }
		} catch (error) {
			task.fail(error)
			throw error
		}
