...
function bytesToBits(data: Uint8Array<ArrayBuffer>): Uint8Array<ArrayBuffer> {
	...
	for (let i = 0; i < data.length; ++i) {
		...
		bits[i * 8 + 1] = (data[i] >> 6) & 1
		bits[i * 8 + 2] = (data[i] >> 5) & 1
		bits[i * 8 + 3] = (data[i] >> 4) & 1
		bits[i * 8 + 4] = (data[i] >> 3) & 1
		bits[i * 8 + 5] = (data[i] >> 2) & 1
		bits[i * 8 + 6] = (data[i] >> 1) & 1
		bits[i * 8 + 7] = (data[i] >> 0) & 1
	}
	return bits
}
...
export const getMnemonic = async (entropy: Uint8Array<ArrayBuffer>): Promise<string[]> => {
	...
	var bits = new Uint8Array(entropyBits.length + entropyBits.length / 32)
	bits.set(entropyBits)
	bits.set(hashBits.subarray(0, entropyBits.length / 32), entropyBits.length)

	const words = []
	const wordsCnt = bits.length / 11
