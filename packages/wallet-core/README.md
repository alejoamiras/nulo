...
- **`Lock.MAX_HOLD_MS`** mirrors `ReadWriteGuard.MAX_READER_DRAIN_MS` — these force-release timers exist to turn a deadlock into a loud log + recovery, not to mask a bug. If one fires in practice, fix the caller.
