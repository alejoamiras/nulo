...
| `src/fee-juice.ts` | `predictedWorstMinFees` — the inclusion-safe worst-case min fee across predicted slots; falls back to the current min fee only when the node lacks the method. |
