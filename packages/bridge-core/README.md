...
| `bun run deploy:sandbox [--smoke] [--keep]` | Boot an OWNED local network (anvil + `aztec start --local-network`, ports from the static window, real-disk data dir, rows in `~/.agents/ports.md`), deploy a generation + three fixture tokens, write the v2 manifest to `sandbox-deploy/sandbox.json`; `--smoke` drives thirteen flows through the production modules (claims public/private/relayed, token+gas, direct gas, exits, first-time registrations, routeless refusal, tampered registration under three fee modes, guardian pause) and prints measured `fjPerTx`/`fjRegister`. `--keep` leaves the network up and prints `SANDBOX_L1_RPC`/`SANDBOX_NODE_URL` to re-attach. |
| `bun run deploy:generation <deploy|pre-create|calibrate>` | The testnet conductor: `deploy` (journalled, resumable; `SEED_TOKENS=<erc20>,…`) writes `apps/tools/public/testnet-bridge.candidate.json`; `pre-create --token <erc20>` adds a token to a candidate (portal clone + hub registration, `--seed-pool`); `calibrate --samples <fees.json>` writes measured fuel budgets. Needs `PRIVATE_KEY` (the pinned testnet signer) + `SEPOLIA_RPC_URL`. |
| `bun run verify:l1 [--config <manifest>]` | On-chain verifier for a v2 manifest: code at every generation address, factory/router cross-bindings + witness shape, per-token portal derivation + frozen registration + live metadata re-derivation, and immutables-masked runtime code hashes against the forge build (`FORGE_BIN` → PATH → the Aztec toolchain). The rest of the deploy/verify/canary scripts are the operator runbook in `.claude/skills/aztec-update/SKILL.md`. |
