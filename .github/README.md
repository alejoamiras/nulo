...
| `pr-extension-smoke-e2e.yml` | `extension-smoke-e2e-status` | dev (cut over 2026-09-09) + main (cut over 2026-09-23) | PR to `main`, OR `e2e:extension-smoke` label, OR `smoke-surface` paths-filter | chrome build + puppeteer smoke (18 files, 67 tests, 7 quarantined) |
| `pr-extension-network-e2e.yml` | `extension-network-e2e-status` | dev (cut over 2026-09-09) + main (cut over 2026-09-23) | PR to `main`, OR `e2e:extension-network` label, OR `extension-network` paths-filter | full network e2e (anvil + Aztec sandbox + playground) |
...
| `publish-packages.yml` | — | — | manual `workflow_dispatch` (`version`, `dry_run`) | stages, packs and digest-checks the three `@alejoamiras/nulo-*` npm packages; with `dry_run` false and the `npm-publish` environment approved, publishes them with provenance (no npm token) |
