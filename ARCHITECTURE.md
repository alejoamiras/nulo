...
| Faucet jsdom smoke | `apps/tools/vitest.e2e.config.ts` (`bun run --cwd apps/tools test:e2e`) | In-process `App.vue` mount with a fake wallet provider — no browser. | Bun | No. |
