...
**On the Send page the fee source follows the transfer's origin** (`apps/extension/src/popup/components/modules/send/fee-privacy.ts`). Every transaction names its fee payer in public: the account's own Fee Juice names the account, the PrivateFPC and a sponsor name that contract. So the default walk is origin-matching payer first, sponsor last — private origin: Private Fee Juice → Fee Juice → Sponsored; public origin: Fee Juice → Private Fee Juice → Sponsored. Under a private origin the wallet *defaults* to the account's own Fee Juice only when the private balance was positively read as `"0"` (and a private-origin mount reads balances fresh); on anything less it takes a sponsor or selects nothing. A private-origin send paid from the account's own Fee Juice — defaulted or hand-picked — renders the fee-payer notice (`send-fee-privacy-notice`); nothing is shown when the payer already matches. The selection is a `computed` over the origin, the committed balance snapshot and the account's pick, never an assigned ref. Send keeps its picks under its own key, one per account × origin (`nulo:ui:sendFeePaymentMethods`, single serialized writer in `fee-send-selection.ts`); the dApp execute window and the authwit popups pass no origin and keep the one-pick-per-account behaviour and key.
