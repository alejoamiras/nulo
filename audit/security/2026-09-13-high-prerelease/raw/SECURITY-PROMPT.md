...
- Crypto misuse: weak algorithms, hardcoded keys, missing IVs, predictable randomness, missing integrity, nonce reuse, key/IV derivation from low-entropy or attacker-influenced input, missing domain separation, KDF parameter weakness, MAC-then-encrypt / unauthenticated fields, timing-variable comparisons on secrets, key material not zeroized, secrets crossing process boundaries in the clear
