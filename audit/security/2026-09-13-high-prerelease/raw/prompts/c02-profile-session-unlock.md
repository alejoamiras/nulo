...
10. Multi-profile: two profiles from the same phrase share the master. Enumerate every master-derived key or MAC and check whether sibling profile A can read/forge anything of profile B (the DEK is the stated separator — verify every place it is actually mixed in).
