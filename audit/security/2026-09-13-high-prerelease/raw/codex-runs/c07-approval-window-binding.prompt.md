...
6. Verify window: `verificationHash` from the URL vs the session's; can a untrusted page open `chrome-extension://<id>/src/popup/index.html#/windows/verify?...` itself (web_accessible_resources says no — confirm) or influence it via a link?
...
9. Window manager races (documented guards) — anything left: two windows for one request, a cancelled window resolving late, focus obtain without authorizationing.
