# Harden Bugs Report
**Repo:** nulo | **Date:** 2026-08-16 | **Effort:** medium | **Run ID:** 2026-08-16-extension-mid | **Models:** Phase 1 map Sonnet ×8 (shared with the parallel quality run); Phase 2 scans Claude Sonnet + Codex gpt-5.6-sol (medium) ×7 clusters; Phase 3 coordinator Sonnet; Phase 4 verifiers Sonnet ×5; orchestration Fable | **Scope:** apps/extension + its workspace dependency closure (wallet-core, wallet-crypto, extension-messaging, aztec-runtime, wallet-bridge, bridge-core [fee-juice.ts only], design, wallet-sdk-schema-patch). Excluded: apps/faucet/playground/landing, generated catalogs, the vendored frozen artifact, bridge-core scripts/ (ops-only), test/e2e code.

## Executive summary

30 consolidated findings survive dedup/refutation/re-anchoring: 6 Critical, 3 Potential Critical, 18 Major, 3 Minor. The top 5 Criticals (B-01 through B-05) all went through a blind Phase 4 verifier pass and came back CONFIRMED at high confidence, with no substantive corrections to the original claims — session unlock/lock RPCs that report success while the wallet stays locked (B-01), a service-worker boot reaper that can fail a transaction that started in the same tick it woke up (B-03), and two independent ways a profile switch can leave stale state governing the active session (B-04, B-05). B-06, the sixth Critical (cross-session verification-hash overwrite), fell outside the medium-effort verifier cap and is consolidated-only. The single largest driver of finding density is a recurring architectural gap rather than 30 unrelated bugs: an async operation captured before a state transition (profile switch, chain purge, offscreen pass timeout, kill-switch toggle, LRU eviction) resolves after that transition and commits its result with no generation/epoch/identity check gating the commit. That shape independently reappears in B-04, B-05, B-07, B-17, B-18, B-20, B-21, and B-29 — eight of the 30 findings — because the codebase has no shared "capture generation → await → compare-and-commit-or-drop" utility; each module reinvented its own epoch variable, and several reinventions forgot the compare-before-commit half.

The most consequential dispute in this run is B-02: codex's own b4 scan pass had filed `executeSendTransaction`'s missing execution-mutex acquisition as a non-finding, citing `transfer-executor.ts`'s documented zero-slot exception as if it covered `dapp-send-executor.ts` too. The Phase 4 verifier re-traced the claim blind, before reading codex's dismissal, and confirmed the bug independently: `dapp-send-executor.ts`'s own file header says dApp sends DO take a slot, contrasting itself with the transfer flow's actual exception — codex conflated two different files. The practical effect is that a connected dApp calling the custom `grantPublicAuthwit` RPC (or racing it against an in-flight send) can run two `simulateTx`/`proveTx` passes concurrently against the same account, which `execution-mutex.ts` exists specifically to prevent. This is a funds-safety-adjacent bug and should be treated with the same urgency as the session-state findings, not deprioritized because a scan leg initially waved it off. On the other end of the spectrum, the b6 storage/backup cluster returned zero surviving findings from either model — every raised lead (Lock force-release double-release, EntityStorage's dual codec policy, the `__proto__` pollution surface, Migrator's "never throws" contract) traced back to an already-pinned characterization test or a closed injection path, which is a genuine signal that this surface's existing hardening holds, not an artifact of under-scanning (both models independently converged on the same non-findings).

Recommended priority order: fix B-01 and B-04 first — both are small, mechanical reorderings (memory-mutation-before-storage-write for B-01, matching the already-correct `refresh()` sibling; a queue reset + try/catch for B-04) with outsized blast radius (false "unlocked"/"locked" state; a balance that silently stops syncing forever). B-02 should ship in the same pass despite being a one-line fix (`runInSlot` wrap) because it is the only Critical with a direct path to on-chain double-submission. B-03 and B-05 are slightly more involved (a boot-cutoff timestamp; a generation counter around a profile-scoped map) but share the same "add a monotonic guard" fix shape as the cross-cutting root cause, so fixing them is a good forcing function for extracting the shared epoch-guard helper the Cross-cutting section recommends — which would also close B-07, B-17, B-18, B-20, B-21, and B-29 by the same mechanism rather than six more one-off patches.

## Methodology

Same shape as the parallel quality run: a shared 8-agent Phase 1 mapping pass split the extension + its workspace dependency closure into 7 clusters by state-owner/call-graph adjacency (b1-session-crypto, b2-transport, b3-pxe-offscreen, b4-execution, b5-pollers, b6-storage-backup, b7-ui-state). Phase 2 ran a Claude Sonnet scan and a Codex gpt-5.6-sol (medium effort) scan independently per cluster — 14 raw reports total, each model blind to the other's output. A Sonnet Phase 3 coordinator deduped, cross-checked disputed claims against source, and re-anchored severities (several raw-severity Criticals were relabeled Potential Critical where the trigger condition, while high-impact, wasn't independently executable/confirmable from this pass alone). Phase 4 ran 5 Sonnet verifiers against the top 5 findings by severity bucket (all Critical), each stating its own blind conclusion from source before being shown the original claim, as an anchoring guard against rubber-stamping.

Deviations from the standard shape, stated honestly: (1) 30 consolidated findings is well above the ~1.2-findings/cluster planning heuristic (30 vs. an expected 6-10) — the coordinator kept them deliberately, driven by one recurring root-cause family (see Cross-cutting §1) and a dense b7 (ui-state) cluster, rather than force-merging structurally distinct bugs into artificially fewer entries; (2) only the top 5 of 6 Criticals got a verifier pass — the medium-effort cap stopped at 5, so B-06 and every Major/Minor finding below it are consolidated-only, not independently re-verified; (3) codex ran at medium effort, not high/max; (4) there was no cross-rebuttal phase at medium effort, so the one dispute that did get resolved (B-02) was resolved incidentally by the Phase 4 verifier's blind trace, not by a dedicated adjudication round.
...
**Counter-example (refined by verifier):** Inject a `BrowserApi` whose `storage.session.set` rejects once (a realistic `QUOTA_BYTES` or MV3 SW-teardown-mid-write failure) → `unlockProfile(id, pw)` resolves a valid `ProfileInfo` → the immediate next `getActiveProfile()`/`getSecret()` throws "Profile locked." `SessionManager.open()` wraps `session.set()` (:221) and the `activeSession` assignment (:223) in ONE try/catch (203-232) with the storage write FIRST — a rejection jumps to the catch and the in-memory assignment never executes, directly contradicting the doc comment ("a broken chrome.storage write at unlock time still leaves the in-memory secret usable"). Symmetric case: reject `remove` once → `lockActiveProfile()` resolves void while the live secret stays in memory. `close()` mirrors `open()`'s inversion: delete (:240) is attempted before the in-memory clear (:241-244).
**Violated invariant:** `session-manager.ts`'s own doc states a storage-write failure should still leave the in-memory secret usable — matching sibling `refresh()`, which mutates memory *before* the storage write (verified genuinely memory-first). `open()`/`close()` invert that ordering.
...
**Verifier findings:** No substantive corrections to the original claim. Additionally verified: `ValueStorage.set/delete` pass the storage-area promise straight through, so the rejection genuinely reaches the swallow; `openSessionVerified` post-open checks only deletion state, never `isActive` (:859); `unlockProfile` returns `ProfileInfo` unconditionally; all 9 `openSessionVerified` call sites are real (280/357/415/504/699/1269/1313/1585/1600); no test exercises a set/delete rejection on open/close — a doc comment implying test coverage pins this behavior references coverage that does not exist, which strengthens rather than weakens the finding.
**Recommended fix (smallest safe):** Memory-first ordering in `open()`/`close()` (mirroring `refresh()`), with the storage write moved into its own non-gating try/catch; plus a post-open `isActive` check in `openSessionVerified` (and a post-close check in `lockActiveProfile`) so a persistence failure surfaces as an RPC error, not silent false success.
...
**Verified:** CONFIRMED (high) — cross-model dispute resolved against codex
...
**Verifier findings:** No corrections — the blind trace matched the finding exactly (call sites, line numbers, missing-slot claim, and the `transfer-executor.ts` distinction). Confirmed `executeAztecSendTx` (~:493) and `executeNoFromSendTx` (~:660) both start with `await deps.lane.acquireSlot` via `runInSlot`; `executeSendTransaction` (:373-460) never calls `runInSlot`/`acquireSlot` and runs straight from `beginJournal` to `buildAndEstimateValidated` to `proveAndSend` on the same pxe/account handles the slot protects. `transfer-executor.ts:14-17`'s "NO execution slot" note is a scoped exception for a genuinely different file (popup transfers) — codex's dismissal conflated the two files.
**Recommended fix (smallest safe):** Wrap `executeSendTransaction`'s body in the existing `runInSlot` scaffold (slot before the journal claim and any PXE work, release in `finally`) — no new mechanism needed.
...
**Verifier findings:** No substantive corrections. On a cold MV3 wake, `reaper.start()` is fired without await (`runtime.ts:268`) and `initWalletSdkHandler` (:295) goes live in the same tick — the RPC listener starts essentially concurrently with the boot sweep. `reap()`'s `getOperations({isTerminal:false})` snapshot has no age/liveness cutoff when unconditional, so it can sweep a row `createOperation` just wrote for the very request that woke the SW. The pipeline's later `markJournal` hits `assertCanTransition("failed", next)`, throws, and is swallowed at `execution-lane.ts:366-367` — the op keeps executing (and can succeed on-chain) while the journal permanently shows `failed`. Strengthening nuance found by the verifier: the root unsynchronized primitive is `getOperations` not taking `transitionLock` (service.ts:450-463), while `createOperation` does (:214).
**Recommended fix (smallest safe):** Capture `bootCutoff = Date.now()` before `reaper.start()`; the unconditional boot sweep only fails rows with `createdAt < bootCutoff`. Consider awaiting the initial sweep before publishing liveness/installing the SDK handler.
...
**Verifier findings:** Confirmed the task-start loop (`balance-job-queue.ts:159-167`) runs before the `try`/`finally` (169/238-242); after a profile switch clears `TaskService`'s map, `startTask(staleId)` throws outside any try/finally, cleanup never runs, and since `enqueue()` gates fresh-mint on `!pendingTasks.has(id)`, every future enqueue coalesces onto the dead entry — a permanent jam for that balance id, not self-healing (nothing else touches the queue's `pendingTasks`). One nuance: `TaskService.onActiveProfileChanged` is gated (profile defined + differing), not literally unconditional, but once the switch fires it clears the entire tasks map with no per-profile filtering, so the finding's phrasing holds in context. Secondary instances confirmed correctly labeled self-recovering/lower-impact: `updateToken`/`parseTokenInterface` mint fresh tasks per call, so the stale-task race there only masks that call's real error (`task.fail` itself throws "Invalid task id" before the original `throw error`).
**Recommended fix (smallest safe):** Have `TokenBalanceService.onActiveProfileChanged` also reset the queue's `pendingTasks`/`queue` (mirroring its own `tokens.clear()`); wrap `syncBatch`'s `startTask`/`startNewTask` calls in `try/catch` and mint a fresh task on failure instead of letting the exception escape before `finally` runs. Defense at both ends.
...
**Verifier findings:** No corrections — all cited line ranges verified exactly; the failing path was confirmed precisely (`session-manager.ts:202-225` onChange at :224 → `profile/service.ts:151` emit → `token-balance/service.ts:240-248`). The race is real: `onActiveProfileChanged` (service.ts:240-248) synchronously clears then *awaits* `getTokensRaw(profile.id)` before repopulating, with no generation guard. `EventHandler.invoke` fires callbacks synchronously without awaiting their async continuations, and the profile facade's `runExclusive` serializes only the emit, not the subscriber's async tail — so rapid switches produce overlapping handler runs, and whichever `getTokensRaw` resolves last wins the map regardless of the active profile. `getTokenBalances` (138-150) and the write/emit gates (103-107) both trust that map.
**Recommended fix (smallest safe):** Add a monotonic profile-generation counter, captured synchronously at handler entry; populate into a temp map and commit into `this.tokens` only if the captured generation is still current after the awaits.
